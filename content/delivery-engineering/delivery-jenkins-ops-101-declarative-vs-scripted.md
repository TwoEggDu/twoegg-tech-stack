...
- Shared Library 配合 Scripted 才能发挥全部能力（[详见 102]({{< relref "delivery-engineering/delivery-jenkins-ops-102-shared-library.md" >}})）
...
只要你需要读外部配置文件、然后**生成数量不确定的 stages**——Declarative 的 `matrix` 撑不住（matrix 的轴是静态的）。这是游戏团队"多产品矩阵"的典型场景，[详见 103]({{< relref "delivery-engineering/delivery-jenkins-ops-103-multi-product-matrix.md" >}})。
...
**这是绝大多数游戏团队应该走的路径**——业务流水线是 Declarative，平台能力是 Scripted。Shared Library 设计[详见 102]({{< relref "delivery-engineering/delivery-jenkins-ops-102-shared-library.md" >}})。
