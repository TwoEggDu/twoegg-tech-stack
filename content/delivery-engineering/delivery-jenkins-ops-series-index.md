...
| 001 | 为什么游戏团队的 Jenkins 是另一个物种 | 和 Web / 服务端比，游戏团队 Jenkins 的结构性差异在哪 |
...
| # | 标题 | 核心问题 |
|---|------|---------|
| 101 | Declarative vs Scripted Pipeline：游戏团队的选型取舍 | 两种语法各自撑住什么规模、什么边界必须切换 |
| 102 | Shared Library 设计：抽象出可复用的游戏构建原语 | 哪些步骤值得下沉、边界怎么切 |
| 103 | 多产品矩阵：一套模板支撑 N 个产品线 | 参数化的三个阶段演进与常见过度抽象 |
| 104 | 多分支流水线：Dev / QA / Release 自动化策略 | 分支策略怎么映射到 Multibranch / Folder |
| 105 | Jenkins 下的并行模式：Parallel / Matrix / Triggers | 扇入扇出在 Jenkins 的三种实现与失败隔离 |
...
| # | 标题 | 核心问题 |
|---|------|---------|
| 201 | Master 的三类瓶颈：连接、内存、磁盘 I/O | 为什么在"大作业 + 大产物"场景下容易崩 |
| 202 | Agent 调度与标签体系 | 标签设计失败会带来什么现象、怎么回到正轨 |
| 203 | Workspace 与产物的磁盘治理 | 几十 G 产物怎么让磁盘不爆 |
| 204 | Jenkins 升级踩坑：JVM / 插件 / 迁移 | 升级路径的四个失败模式与回滚预案 |
| 205 | Jenkins 自身的可观测性：监控与告警 | Jenkins 自己挂了谁来发现 |
...
| # | 标题 | 核心问题 |
|---|------|---------|
| 301 | Unity License 池管理：稀缺资源的治理 | License 不够用、泄漏、占用不释放的三类故障 |
| 302 | 大仓库在 Jenkins 下的 Workspace 策略 | Git LFS / 浅克隆 / 缓存仓在真实大仓下的取舍 |
| 303 | 符号表与崩溃栈：IL2CPP 产物的符号链路 | 线上崩溃栈还原怎么自动化 |
| 304 | 多平台并行打包与隔离 | 同时出 iOS / Android / WebGL 的资源冲突与隔离 |
| 305 | IL2CPP 构建的时间与内存特征 | 为什么 IL2CPP 构建机最容易挂 |
