...
- Master 磁盘有限，不能堆积 GB 级符号表（[详见 203]({{< relref "delivery-engineering/delivery-jenkins-ops-203-disk-governance.md" >}})）
...
- **尝试 1**：rebuild 同 commit → 新 dSYM 和原 binary 地址不一致，无法用于符号化（[详见 001 总论的"版本错位"陷阱]({{< relref "delivery-engineering/delivery-jenkins-ops-001-why-different.md" >}})）
