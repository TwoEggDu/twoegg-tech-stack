...
ECMA-335 基础层第 3 篇（ECMA-A6）讲清了 CLI 类型系统的内存语义：引用类型在托管堆上分配，生命周期由运行时管理；值类型内联存储，赋值即复制。规范要求运行时提供自动内存管理（Partition I §12.6 "Memory model and optimizations"），但不约束具体实现——可以是分代式、标记-清扫、引用计数，甚至完全不回收。
