...
如果只记名字，`Minimal / Low / Medium / High` 很容易被脑补成同一条滑杆。

但如果换成"构建机会怎么决定每个程序集怎么处理"，区别会立刻清楚很多。
...
{
    if (backend == Mono && level == Disabled)
    {
        CopyAsIs(assembly);
        continue;
    }
    ...
    if (level == Minimal)
    {
        if (assembly.IsClassLibrary || assembly.IsUnityEngineAssembly || assembly.IsWindowsRuntimeAssembly)
            RunUnityLinker(assembly, ruleSet: Minimal);
        else
            CopyAsIs(assembly);
    ...
    RunUnityLinker(assembly, ruleSet);
}
```

这段伪代码最关键的不是细节，而是这两个分支：

- `Minimal`：先判断"这个程序集属不属于那一小批要处理的范围"
- `Low / Medium / High`：已经默认进入"把程序集交给 linker 按可达性判断"的主线

也就是说：

`Minimal` 和 `Low` 的差别，首先不是"同样处理全量程序集，但一个删得轻一个删得重"，而是"有些程序集在 Minimal 下根本就还没上那条主线"。`

## 再看一个具体例子，为什么从 Minimal 切到 Low 常常像踩台阶
...
- `UnityEngine.UI.dll`
- `Assembly-CSharp.dll`
- `ThirdParty.Plugin.dll`

如果把 `Minimal` 和 `Low` 的处理方式压成对照，大概是这样：

| 程序集 | `Minimal` 下更像什么 | `Low` 下更像什么 |
| --- | --- | --- |
| `netstandard.dll` | 进入 linker | 进入 linker |
| `UnityEngine.UI.dll` | 进入 linker | 进入 linker |
| `Assembly-CSharp.dll` | 往往直接复制 | 进入 linker |
| `ThirdParty.Plugin.dll` | 往往直接复制 | 进入 linker |

这张表非常重要。

因为真实项目里最容易出事的，恰恰通常不是：

- `netstandard.dll`
- `UnityEngine` 那些相对标准、相对可预测的程序集
...
所以一旦从 `Minimal` 切到 `Low`，很多团队体感上会像突然跨了一个台阶：

`以前主要还是"系统库在裁"，现在开始轮到你自己的程序集和第三方程序集真的上场了。`

这就是为什么我会说：

`Minimal` 更像"有限范围处理"，而 `Low` 才是你真正开始接受"项目自身依赖图要经得起可达性裁剪"这件事。`

## UnityLinker 是怎么把这些级别真正落地的

如果只看文档，很多人会觉得这些词还是有点虚。

但如果顺着当前实现往下看，这几个级别最终会被映射成明确的 `UnityLinker` 规则集：
...
如果把这件事压成一段更容易记的伪代码，大概就是：

```csharp
var effectiveLevel = selectedLevel;

if (backend == IL2CPP && effectiveLevel == Disabled)
    effectiveLevel = Minimal;

if (backend == Mono && effectiveLevel == Disabled)
{
    CopyAllAssembliesAsIs();
    return;
}

RunManagedStripping(effectiveLevel);
```

这段伪代码想说明的就一句话：

`Mono 下的 Disabled 和 IL2CPP 下"你以为自己选了 Disabled"根本不是同一件事。`
...
把这个迁移思路压成伪代码，可以近似理解成：

```csharp
if (project.HasSerializedManagedStrippingLevel)
    return project.SerializedManagedStrippingLevel;

if (project.IsLegacyProject && backend == IL2CPP)
    return legacyDefaultLevel;

return currentEditorDefaultLevel;
```

这段伪代码解释了一个很现实的问题：

`同样是"我没动这个选项"，新项目和旧项目、老版本工程和新版本工程，最后吃到的默认值并不一定一样。`
...
如果你想把这句话记得更牢，我建议直接记这个近似心智模型：

```text
Minimal = 先只处理一小批 Unity 认为更标准的程序集
Low     = 开始让你自己的程序集也正式进入 linker 主线
```
...
- 上一篇：<a href="{{< relref "engine-toolchain/unity-stripping-01-what-gets-stripped.md" >}}">Unity 裁剪 01｜Unity 的裁剪到底分几层</a>
