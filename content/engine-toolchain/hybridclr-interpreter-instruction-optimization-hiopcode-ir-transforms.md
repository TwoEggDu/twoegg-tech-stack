...
> **数据来源声明**：以下数字均引自 HybridCLR 官方文档（`hybridclr.cn`），属于厂商声称的性能提升，**不是独立 benchmark 验证结果**。社区版源码可以验证前面几节的分析；商业版优化的具体实现不公开，性能数据无法独立复现。

| 指标 | 官方声称 |
|------|----------|
| 数值计算性能提升（vs 社区版） | 280% ~ 735% |
| `typeof` 操作性能提升 | > 1000% |
| 编译后指令数量减少 | 到社区版的 1/4 ~ 1/2 |
