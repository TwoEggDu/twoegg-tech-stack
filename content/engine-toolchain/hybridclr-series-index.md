...
> 如果你已经熟悉 ECMA-335 metadata 和 CIL 指令集，可以跳过前置层直接进入基础层。

Pre-A. [HybridCLR 前置篇｜CLI Metadata 基础：TypeDef、MethodDef、Token、Stream 到底是什么]({{< relref “engine-toolchain/hybridclr-pre-cli-metadata-typedef-methoddef-token-stream.md” >}})
   给”metadata”这个贯穿全系列的核心词一个实体定义。后面所有涉及 token、image、method body 的讨论都建在这层之上。

Pre-B. [HybridCLR 前置篇｜CIL 指令集与栈机模型：ldloc、add、call 到底在做什么]({{< relref “engine-toolchain/hybridclr-pre-cil-instruction-set-stack-machine-model.md” >}})
   先知道 CIL 长什么样、栈机怎么执行、为什么直接解释慢，后面的 transform 和 HiOpcode 才不是空中楼阁。

### 基础层（HybridCLR runtime 主链）
...
Bridge-C. [HybridCLR 桥接篇｜ABI 与跨边界调用：为什么 interpreter 调 AOT 需要一层 bridge]({{< relref “engine-toolchain/hybridclr-bridge-abi-cross-boundary-calling-convention.md” >}})
   读完工具链知道 MethodBridge 在生成什么，这篇告诉你为什么要生成——ARM64 AAPCS 参数传递 vs StackObject 扁平布局的冲突。
...
Bridge-D. [HybridCLR 桥接篇｜IL2CPP 泛型共享规则：引用类型共享 object，值类型为什么不能]({{< relref "engine-toolchain/hybridclr-bridge-il2cpp-generic-sharing-rules.md" >}})
   补上 HCLR-2 到 HCLR-10 之间的断层：IL2CPP 原本就做了部分 generic sharing，引用类型共享 object，值类型独立实例。不先知道这层，FGS 和 DisStripCode 的底层原因就接不上。
...
> 下面两篇为 HCLR-25~30 的深度层搭桥。如果你已经熟悉解释器 dispatch 模型和 IL2CPP GC 机制，可以跳过。

Bridge-E. [HybridCLR 桥接篇｜解释器基础：dispatch loop、stack vs register、IR 是什么]({{< relref "engine-toolchain/hybridclr-bridge-interpreter-basics-dispatch-stack-register-ir.md" >}})
   给 HCLR-27（指令优化）搭桥：switch dispatch、stack machine vs register machine、IR 和常见优化术语。

Bridge-F. [HybridCLR 桥接篇｜IL2CPP GC 模型：BoehmGC、GC root、write barrier 与解释器的关系]({{< relref "engine-toolchain/hybridclr-bridge-il2cpp-gc-model-boehm-root-write-barrier.md" >}})
   给 HCLR-28（热重载）和 HCLR-30（Incremental GC）搭桥：保守式 GC、GC root 注册、write barrier 机制。

### 商业功能深度研究子系列（HCLR-25 ~ HCLR-30）

> 下面 6 篇围绕 HybridCLR 三个商业版本（专业版 / 旗舰版 / 热重载版）的核心功能展开。目标不是做接入教程，而是把"商业版到底改了什么、怎么实现的"讲到源码/逆向/架构推导的深度。

25. [HybridCLR DHE 内部机制｜dhao 文件格式、差分算法与函数级分流实现]({{< relref "engine-toolchain/hybridclr-dhe-internal-dhao-format-diff-algorithm-function-routing.md" >}})
   从 HCLR-11 概念篇接续，下到 dhao 文件结构、IL 级差分算法、运行时 methodPointer 切换和 MetaVersion 工作流。

26. [HybridCLR DHE 函数注入与脏函数传染｜为什么改一个函数会导致一片函数走解释器]({{< relref "engine-toolchain/hybridclr-dhe-function-injection-dirty-contagion.md" >}})
   拆清"脏函数传染"的机制——直接传染 vs 间接传染，注入规则怎么打断传染链，注入的 ~30% 代码膨胀和短函数性能代价。

27. [HybridCLR 解释器指令优化｜标准优化与高级优化在 HiOpcode 层到底做了什么]({{< relref "engine-toolchain/hybridclr-interpreter-instruction-optimization-hiopcode-ir-transforms.md" >}})
   从 1000+ HiOpcode 指令集、CIL 栈机到寄存器 IR 转换、社区版 8 种 transform 优化，到商业版 4 种追加优化（数值计算提升 280%-735%）。

28. [HybridCLR 程序集热重载｜IL2CPP 为什么不能卸载程序集，热重载版怎么做到的]({{< relref "engine-toolchain/hybridclr-assembly-hot-reload-unload-metadata-reclaim.md" >}})
   从 IL2CPP 的结构性障碍（全局表只增不减、MetadataPool 无单对象释放）推导热重载版需要改什么，GC 配合、重加载状态恢复和工程边界。

29. [HybridCLR 代码加密与访问控制｜DLL 加密怎么与解释器配合，访问控制策略拦在哪一层]({{< relref "engine-toolchain/hybridclr-code-encryption-access-control-policy.md" >}})
   7 层加密体系、vmSeed 参数机制、延迟解密防 dump、访问控制 XML → binary 策略、Obfuz 互补关系。

30. [HybridCLR 加载加速、metadata 内存优化与 Incremental GC 适配｜商业版在运行时资源消耗上做了什么]({{< relref "engine-toolchain/hybridclr-load-acceleration-metadata-memory-incremental-gc.md" >}})
   Assembly::Load 优化到原来 30%、metadata 内存 4.7x→2.9x、社区版 v4.0.0 起已支持 Incremental GC 的 WriteBarrier 机制。
...
### 25. 你想知道 DHE 的 dhao 文件到底长什么样、差分算法怎么判断方法变了

先看：

- [HybridCLR DHE 内部机制｜dhao 文件格式、差分算法与函数级分流实现]({{< relref "engine-toolchain/hybridclr-dhe-internal-dhao-format-diff-algorithm-function-routing.md" >}})

### 26. 你用了 DHE 但发现改一个函数后一大片函数都走了解释器

先看：

- [HybridCLR DHE 函数注入与脏函数传染｜为什么改一个函数会导致一片函数走解释器]({{< relref "engine-toolchain/hybridclr-dhe-function-injection-dirty-contagion.md" >}})

### 27. 你想知道 HybridCLR 解释器的 HiOpcode 到底做了什么优化

先看：

- [HybridCLR 解释器指令优化｜标准优化与高级优化在 HiOpcode 层到底做了什么]({{< relref "engine-toolchain/hybridclr-interpreter-instruction-optimization-hiopcode-ir-transforms.md" >}})

### 28. 你想知道热重载版怎么做到卸载并重新加载程序集

先看：

- [HybridCLR 程序集热重载｜IL2CPP 为什么不能卸载程序集，热重载版怎么做到的]({{< relref "engine-toolchain/hybridclr-assembly-hot-reload-unload-metadata-reclaim.md" >}})

### 29. 你想给热更 DLL 加密、或用访问控制限制第三方代码

先看：

- [HybridCLR 代码加密与访问控制｜DLL 加密怎么与解释器配合，访问控制策略拦在哪一层]({{< relref "engine-toolchain/hybridclr-code-encryption-access-control-policy.md" >}})

### 30. 你觉得 Assembly::Load 太慢、metadata 占内存太多、或想了解 Incremental GC 适配

先看：

- [HybridCLR 加载加速、metadata 内存优化与 Incremental GC 适配｜商业版在运行时资源消耗上做了什么]({{< relref "engine-toolchain/hybridclr-load-acceleration-metadata-memory-incremental-gc.md" >}})
...
商业功能深度研究：
- DHE 内部机制：dhao 文件格式、IL 级差分算法、函数级分流、MetaVersion 工作流
- DHE 函数注入与脏函数传染：直接/间接传染机制、注入规则阻断、代码膨胀与性能代价
- 解释器指令优化：1000+ HiOpcode 指令集、CIL→寄存器 IR 转换、社区版 8 种 + 商业版 4 种优化
- 程序集热重载：IL2CPP 结构性障碍、MetadataCache 可逆注册、GC 配合、重加载状态恢复
- 代码加密与访问控制：7 层加密体系、延迟解密、vmSeed 机制、XML 访问策略
- 加载加速与内存优化：Assembly::Load 30%、metadata 4.7x→2.9x、Incremental GC WriteBarrier

到这一步，这组文章从基础原理、高级能力、真实案例、AOT 泛型工程化到商业功能深度研究，已经形成完整的知识体系。
