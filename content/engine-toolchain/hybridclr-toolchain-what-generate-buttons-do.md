...
如果前两篇在讲 runtime 主链怎么跑、AOT 泛型问题为什么会出现，那么这一篇其实是在讲：为什么 HybridCLR 必须先有一条足够完整的 build-time 流水线，runtime 那条链才有可能成立。

三篇合起来，才比较接近 HybridCLR 在工程里的完整样子。

## 系列位置

- 上一篇：<a href="{{< relref "engine-toolchain/hybridclr-aot-generics-and-supplementary-metadata.md" >}}">HybridCLR AOT 泛型与补充元数据｜为什么代码能编译，到了 IL2CPP 运行时却不一定能跑</a>
