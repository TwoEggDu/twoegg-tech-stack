...
> **本文明确不展开的内容：**
> - 各 JIT 编译器内部优化（RyuJIT、Mini 的优化 pass 细节在各自的专题篇展开）
> - 具体平台汇编输出（x64 / ARM64 / WASM 的机器码细节不在本文范围）
> - benchmark 数据（五种执行策略的性能对比数据不在本文提供）

![5 个 Runtime 的方法执行路径](../../images/runtime-ecosystem/cross-method-execution-paths.svg)

*图：同一份 CIL 方法体，5 条执行路线的核心差异在于代码转换发生在什么时候。*
...
| **源码锚点** | `src/coreclr/jit/importer.cpp` | `mono/mini/method-to-ir.c` | il2cpp.exe (closed source, output: `GenericMethods.cpp`) | `hybridclr/transform/Transform.cpp` | `src/runtime/interp/hl_transformer.cpp` |
