...
调用方在具体的 `ResourcePackage` 实例上发起请求。Package 内部持有一个 `PackageManifest`，Manifest 维护一张核心字典和一个列表：

- `_assetDic`：`Dictionary<string, PackageAsset>`，key 是 address 字符串
- `_bundleList`：`List<PackageBundle>`，BundleID 就是列表下标，直接索引

查找路径：
```
address → _assetDic[address] → PackageAsset → BundleID → _bundleList[bundleID] → PackageBundle
```

没有 hash bucket，没有 entry offset，没有 Base64 解码。Manifest 反序列化后直接就是字典加列表，定位就是一次字典查找加一次列表下标访问。
...
`ResourcePackage` 在拿到 `AssetInfo` 后，创建一个 `ProviderBase`（如 `BundledAssetProvider`）。ProviderBase 通过 YooAsset 内部的 `AssetSystem`（和 Addressables 的 `ResourceManager` 是完全不同的类）获取或创建 `BundleLoaderBase` 实例。

Loader 的选择不是通过字符串匹配，而是由 PlayMode 在初始化时决定：

- `HostPlayMode` → `AssetBundleWebLoader`
- `OfflinePlayMode` → `AssetBundleFileLoader`
- `EditorSimulateMode` → `EditorSimulateLoader`

每个 bundleID 在运行时只有一个 Loader 实例。Loader 字典以 bundleID 为 key，直接索引。

依赖处理是扁平的：ProviderBase 从 `AssetInfo.DependBundleIDs` 拿到依赖列表（一个整数数组），为每个依赖获取或创建 Loader，等所有依赖 Loader 就绪后启动主 bundle Loader。没有递归——依赖结构只有一层。
...
flowchart LR
    ...
    subgraph YooAsset
        ...
        B4 --> B5["创建 ProviderBase"]
        B5 --> B6["AssetSystem<br/>按 bundleID 索引 Loader"]
        B6 --> B7["AssetBundleFileLoader / WebLoader<br/>加载 bundle"]
...
| 维度 | Addressables | YooAsset |
|------|-------------|---------|
| 调度中心 | 全局 `ResourceManager` 单例 | `ResourcePackage` 各自独立（内部由 `AssetSystem` 协调） |
...
**ProviderBase 层**：每次 `LoadAssetAsync` 都创建一个独立的 `ProviderBase`。即使两次加载同一个 address，也是两个独立的 provider。

**BundleLoader 层**：`BundleLoaderBase` 维护自己的引用计数，记录被多少个 ProviderBase 引用。

```
// 两次加载同一个 address → 两个独立 provider，共享 bundleLoader
handle1 = package.LoadAssetAsync<T>(addr);  // provider1, bundleLoader refcount = 1
handle2 = package.LoadAssetAsync<T>(addr);  // provider2, bundleLoader refcount = 2
```

Release 链：
```
AssetOperationHandle.ReleaseInternal()
  → ProviderBase.ReleaseHandle()          // ProviderBase 递减自身引用计数
  → BundleLoaderBase.Release()            // 递减 BundleLoader 的引用计数
  → refcount == 0 → BundleLoaderBase.TryDestroyLoader()
...
| 维度 | Addressables | YooAsset |
|------|-------------|---------|
| 引用计数粒度 | operation 级别（同 key 共享 operation） | 双层：ProviderBase 独立 + BundleLoader 引用计数 |
| 同 key 二次加载 | 复用已有 operation，refcount++ | 创建新 ProviderBase，bundleLoader refcount++ |
...
| 追踪粒度 | 只能追踪到 operation 级别，多个 handle 共享一个 operation 时难以区分谁在持有 | 每个加载请求是独立的 ProviderBase，可以逐个追踪 |
| 典型泄漏表现 | bundle 永不卸载，内存只增不减 | 同上，但 `ForceUnloadAllAssets` 提供了兜底手段 |

工程影响：Addressables 的 operation 共享机制减少了重复操作对象的创建，但代价是"谁在持有这个资源"变得更难追踪——多个 handle 背后是同一个 operation，Profiler 里看到的 refcount 是合并后的数字。YooAsset 的每请求独立 ProviderBase 在诊断时更透明，你能清楚地看到每个加载请求对应的 provider 和它引用了哪些 Loader。但这也意味着更多的 provider 对象创建和管理开销。
...
`AssetOperationHandle` 包裹一个 `ProviderBase`，`ProviderBase` 继承自 `GameAsyncOperation`——YooAsset 自己的异步操作基类。
...
源码位置：
- `Runtime/AssetSystem/Handles/AssetOperationHandle.cs`
...
| 维度 | Addressables | YooAsset |
|------|-------------|---------|
| Handle 类型 | `AsyncOperationHandle<T>`（结构体，包裹 `AsyncOperationBase<T>`） | `AssetOperationHandle`（类，包裹 `ProviderBase`） |
| Completed 回调 | 支持 | 支持 |
...
| 需要精细追踪每个加载请求的生命周期 | YooAsset | 每次 LoadAssetAsync 创建独立 ProviderBase，比 Addressables 的 operation 共享模型更容易追踪谁在持有什么 |
