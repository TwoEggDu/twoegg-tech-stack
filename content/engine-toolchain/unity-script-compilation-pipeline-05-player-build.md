...
**`global-metadata.dat` 和 `GameAssembly`** 是 IL2CPP 打包的两大产物：前者存类型元数据，后者是原生代码库。两者在运行时的分工见 [IL2CPP 运行时地图]({{< relref "engine-toolchain/il2cpp-runtime-map-global-metadata-gameassembly-libil2cpp.md" >}})。
...
- 上一篇：[Unity 脚本编译管线 04｜编译卡死怎么看：从日志定位卡在哪一环]({{< relref "engine-toolchain/unity-script-compilation-pipeline-04-debug-hang.md" >}})
- 下一篇：[Unity 脚本编译管线 06｜.asmdef 设计：如何分包才能让增量编译更快]({{< relref "engine-toolchain/unity-script-compilation-pipeline-06-asmdef-design.md" >}})
- 延伸阅读：[IL2CPP 运行时地图｜global-metadata.dat、GameAssembly、libil2cpp 到底各管什么]({{< relref "engine-toolchain/il2cpp-runtime-map-global-metadata-gameassembly-libil2cpp.md" >}})
