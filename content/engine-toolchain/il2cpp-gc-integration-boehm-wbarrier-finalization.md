...
LeanCLR 不自带 GC。作为约 600KB 的纯 C++17 解释器运行时，LeanCLR 把内存管理完全委托给宿主环境。宿主（浏览器的 JS 引擎、小游戏运行时等）提供自己的 GC 或手动内存管理。

这种策略避免了在轻量 runtime 中实现 GC 的复杂度（无论保守式还是精确式），代价是无法独立控制收集策略。宿主的 GC 特性直接决定了 LeanCLR 应用的内存行为。
