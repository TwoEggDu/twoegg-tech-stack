...
> **本文明确不展开的内容：**
> - Mono 源码逐文件分析（本文按模块拆解架构，不深入单个源文件的实现细节）
...
这一层的职责和 LeanCLR 的 `metadata/` 目录功能一致：都是在做 ECMA-335 Partition II 定义的 metadata table 解析。但 Mono 的 metadata 模块还额外承担了程序集搜索、版本匹配、GAC（Global Assembly Cache）查找等职责——这些在 LeanCLR 中被刻意省略了。（关键源文件：`mono/metadata/image.c` 程序集加载，`mono/metadata/class.c` 类型构建，`mono/metadata/loader.c` 方法解析）
...
这是 Mono 和 CoreCLR（RyuJIT）在同一层面上的对应物。两者的输入都是 IL，输出都是 native code，但内部的 IR 设计和优化策略完全不同。（关键源文件：`mono/mini/method-to-ir.c` IL→IR 转换，`mono/mini/mini.c` 管线协调，`mono/mini/mini-amd64.c` / `mono/mini/mini-arm64.c` 平台代码生成）
...
这个模块是本系列 MONO-C2 的重点，这里只标注位置。（关键源文件：`mono/mini/interp/interp.c` 解释器主循环，`mono/mini/interp/transform.c` IL→InterpInst 变换）
...
这和 CoreCLR 的 GC 在设计目标上一致（都是精确式分代 GC），但 CoreCLR 的 GC 规模和复杂度远超 SGen——CoreCLR 支持 Workstation/Server 两种模式、后台 GC、Pinned Object Heap 等高级特性。（关键源文件：`mono/sgen/sgen-gc.c` GC 核心逻辑，`mono/sgen/sgen-nursery-allocator.c` 新生代分配）
...
在早期版本中，io-layer 试图在 Unix 上模拟 Win32 API 的行为。后来的版本逐步移除了这种设计，改为直接使用 POSIX API 并在上层做抽象。（关键源文件：`mono/utils/mono-threads.c` 线程管理，`mono/utils/mono-os-mutex.h` 互斥锁抽象）
