...
**LeanCLR 当前是 stub GC（malloc-only），不是"委托给宿主"。** LeanCLR 作为一个面向 H5/小游戏的纯 C++17 runtime（约 600KB），当前开源版本的 GC 实现是一个 stub：通过 `GeneralAllocation::alloc` 走系统 `malloc` 分配，不做任何回收。对象一旦分配，除非进程退出，否则不会被释放。架构层面已预留 `write_barrier` 接口，**设计目标**是未来支持精确协作式 GC（Universal 版）和保守式 GC（Standard 版），但当前实现并未达成。
