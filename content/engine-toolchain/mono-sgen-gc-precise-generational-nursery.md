...
**LeanCLR 当前的 stub GC 策略。** LeanCLR 作为约 600KB 的纯 C++17 runtime，**当前**开源实现走 `malloc` 不回收，不是"委托给宿主"。架构层面预留了 `write_barrier` 接口，**设计目标**是 Universal 版做精确协作式 GC、Standard 版做保守式 GC，但两条路线都未达成。把 LeanCLR README 里的"开发中"路线图当作"当前能力"是常见的误读。
