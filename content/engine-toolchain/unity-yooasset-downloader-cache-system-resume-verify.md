---
date: "2026-04-13"
title: "YooAsset 的下载器和缓存系统：ResourceDownloaderOperation、CacheFileSystem 和断点续传"
description: "沿 YooAsset 源码拆开下载队列、并发控制、校验重试和断点续传的实现路径，以及 CacheFileSystem 的磁盘结构和版本清理机制。"
...
Yoo-01 在讲 HostPlayMode 的加载路径时提到——如果缓存未命中，`AssetBundleLoader` 会触发下载，下载完成后写入缓存再加载 bundle。但"下载"和"缓存"内部到底怎么工作，没有展开。
...
`CacheFileSystem 在磁盘上长什么样？怎么区分"已下载"和"下载了一半"？`
...
遍历 PackageManifest._bundleList
  → 对每个 PackageBundle：
    → 问 CacheFileSystem：这个 bundle 的 FileHash 对应的缓存文件存在吗？
    → 存在且校验通过 → 跳过
    → 不存在 或 校验不通过 → 加入下载列表
```
...
YooAsset 2.x 中，单文件下载的核心实现在 `DownloadHandlerFileOperation` 及其相关类中。内部使用 Unity 的 `UnityWebRequest` 发起 HTTP 请求。
...
## 四、CacheFileSystem 的磁盘结构

下载完成后，文件要落盘到缓存目录。这一节拆开 `CacheFileSystem` 的磁盘布局和索引机制。
...
`__info` 的作用是让 `CacheFileSystem` 在不读取 `__data` 文件内容的情况下，快速判断缓存是否有效。启动时重建缓存索引只需要读 `__info`，不需要校验整个 bundle 文件。

### 缓存索引的构建

`CacheFileSystem` 在初始化时扫描缓存目录，为每个有效的缓存条目建立内存索引：
...
**已下载且验证通过：** 缓存目录中有完整的 `{BundleGUID}/` 子目录，包含 `__data` 和 `__info` 两个文件，且 `__info` 中记录的元数据和 manifest 中的对应 `PackageBundle` 信息一致。这种文件会被 `CacheFileSystem` 索引，`LoadAssetAsync` 可以直接从 `__data` 加载。

**下载了一半：** 只有一个 `.temp` 临时文件，存在于临时下载目录中（不在 `CacheFiles/` 下）。`CacheFileSystem` 不索引临时文件。下次创建 `ResourceDownloaderOperation` 时，这个 bundle 会出现在下载列表中，下载器检测到对应的 `.temp` 文件后尝试续传。

**下载完成但校验失败：** 临时文件被删除，不留任何痕迹。这个 bundle 等同于"完全没下载"，下次会从头开始。

```mermaid
flowchart TD
    A["CacheFileSystem 初始化<br/>扫描 CacheFiles/ 目录"] --> B{"子目录内<br/>__data + __info<br/>都存在？"}
    ...
    G["下载流程中的<br/>.temp 文件"] --> H["不在 CacheFiles/ 目录<br/>CacheFileSystem 不可见"]
...
2. **缓存是自管的，不依赖引擎。** `CacheFileSystem` 的 `__data` + `__info` 双文件结构，让缓存的有效性判断和清理都在应用层完成，项目可以做到文件级的精细控制。
