...
> **关于本文数字的重要说明：**
> 本文涉及大量体积数字（50MB / 600KB / 300KB 等）。这些数字在不同平台、构建模式、裁剪级别、压缩阶段下会显著变化。请把这些数字理解为 **示意范围** 而不是 benchmark 事实：
>
> - **CoreCLR 数字**：基于 .NET 8 self-contained x64 Linux 默认构建（runtime + BCL + 应用），未启用 PublishTrimmed
> - **Mono 数字**：基于 Unity 2022 LTS 编辑器内嵌的 Mono runtime（不含 BCL）
> - **IL2CPP 数字**：runtime 部分指 `libil2cpp.a` 静态库（Android arm64 Release 构建），GameAssembly 体积取决于具体项目
> - **HybridCLR 数字**：基于社区版主包，Unity 2022 LTS + IL2CPP backend，arm64 Release
> - **LeanCLR 数字**：基于 v0.1.x 单线程版 release 构建，wasm 目标 + 默认 emcc 优化
> - **WASM 数字**：传输大小是 Brotli 压缩后；解压后实际加载体积约为传输大小的 2-3 倍
>
> 项目实际数字会因 Unity 版本、平台编译器、stripping 级别、是否启用 LTO 等因素浮动 30%-100%。请以本文数字作为量级判断（KB / MB / 数十 MB），不要作为精确测量结论。
