...
<!-- DATA-TODO: 下面这张贴图规格表是"工程建议值"而不是"实测最优值"。建议：挑一个真实项目（原神 / 星穹铁道 / 自研项目），在四档代表机上做 A/B 测试——用表格里的规格 vs 下压/上调一档，看视觉差异和帧率差异。至少在表下加一行"数据来源：基于多个移动端项目的美术规格汇总整理，具体项目需要按视觉风格和目标设备自测校准"。每档可提供一组 before/after 截图存 static/images/perf/texture-spec-comparison/。 -->
| 贴图类型 | 高档 | 中档 | 低档 | 极低档 |
|---------|------|------|------|--------|
...
<!-- DATA-TODO: "2048 ASTC 4x4 贴图解压后约 16MB"是计算结果（2048*2048*0.5 byte 解压后 = 2 MB 如果保持 ASTC，如果 GPU 解压到 RGBA 则是 16MB），"极低档贴图预算 150-200 MB"是工程经验估算。建议：(1) 前者改写成计算公式+条件（"如果 GPU 驱动在加载时解压为 RGBA32，占 16MB；保持 ASTC 压缩驻留则约 2MB，具体看设备 GPU 驱动行为"）；(2) 后者加一句"基于 iPhone 6 / Redmi 4A 级别 1GB RAM 设备的 Unity Memory Profiler 观察，不同项目差异 ±50MB 都属正常"。 -->
...
芯片档位判断的详细代码见：
[主流芯片档位参考表：四档判断依据与代码]({{< relref "performance/device-tier-chip-reference-four-tiers.md" >}})

热机状态如何影响运行时降档见：
[URP 深度平台 04｜热机后的质量分档]({{< relref "rendering/urp-platform-04-thermal-and-dynamic-tiering.md" >}})
