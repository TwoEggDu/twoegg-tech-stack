...
1. [从型号表到能力指纹：Android 与 PC 的分档判断怎么设计]({{< relref "performance/from-model-table-to-capability-fingerprint-android-and-pc-tiering.md" >}})
   规则模型层：型号表为什么会失效，硬门槛 + 能力评分 + 黑白名单 + 玩家覆盖怎么组合。
2. [Unity 分档判断的工程实现：可用信号、Android 碎片化与本地+云端双层架构]({{< relref "performance/game-performance-tier-detection-design.md" >}})
   工程实现层：Unity SystemInfo 能读什么、哪些信号可信、本地兜底 + 云端特例表怎么分工。
3. [主流芯片档位参考表：四档判断依据与代码]({{< relref "performance/device-tier-chip-reference-four-tiers.md" >}})
   芯片级落地：Apple / Qualcomm / MediaTek / Huawei 四家的档位表和完整检测代码。
4. [URP 深度平台 02｜多平台质量分级：三档配置的工程实现]({{< relref "rendering/urp-platform-02-quality.md" >}})
   接入 URP：Quality Settings 和 URP Asset 怎么按档位配对。

这一段先解决四个根问题：

- 你到底该按型号表分，还是按能力指纹分（规则模型）。
- 这套规则落到 Unity 里该读哪些信号、避开哪些不可信字段（工程实现）。
- 具体到主流芯片，每一颗应该归到哪一档、代码怎么写（芯片级）。
- 三档什么时候够用，什么时候应该扩成四档（档位粒度）。
...
3. [四个档位的玩家应该感受到同一款游戏：体验一致性设计]({{< relref "performance/device-tier-experience-consistency.md" >}})
   含视觉 / 听觉 / 动画三个身份维度、典型反面案例（通用模板砍出来的"变味"）、以及开发期如何预防身份腐化。
4. [策划怎么验收分档一致性：低中高档分别该保什么，不该保什么]({{< relref "performance/game-performance-design-tier-acceptance.md" >}})
   面向策划的验收视角：四类关键场景、三层差异记录（身份层 / 可玩层 / 装饰层）、上线前检查清单。
5. [分档的隐藏成本：什么情况下多一档反而是负收益]({{< relref "performance/device-tier-hidden-cost.md" >}})
   含边界设备识别抖动、配置腐化的检测和止损、已经做多了之后怎么优雅缩档。

这一段更关心的是：

- 每一档到底交付什么，不只是关几个开关。
- 当预算不够时，哪些效果该先砍，哪些该最后砍。
- 低档玩家还能不能感受到"这是同一款游戏"——视觉、听觉、动画都算身份。
- 策划验收时怎么把"变味 vs 正常降级"讲清楚，不再反复靠感觉争论。
- 多一档到底是在换收益，还是在扩大维护面（含边界抖动、配置腐化、缩档退路）。
...
- 概念都懂了，但不知道 Unity 代码里该怎么写：
  先看 [Unity 分档判断的工程实现]({{< relref "performance/game-performance-tier-detection-design.md" >}}) 和 [主流芯片档位参考表]({{< relref "performance/device-tier-chip-reference-four-tiers.md" >}})。
- 策划要验收分档一致性，但说不清楚"变味"和"正常降级"的边界：
