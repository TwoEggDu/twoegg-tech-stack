...
- `Full variant space → After settings filtering` 之间减少：枚举阶段（usedKeywords + 设置过滤）
- `After settings filtering → After built-in stripping` 之间减少：Unity 内置 stripping
- `After built-in stripping → After scriptable stripping` 之间减少：SRP + 自定义 stripping
...
{
    yield return svc.WarmUpProgressively(16); // 每帧预热 16 个变体，返回 true 表示全部完成；或直接调用 WarmUp() 一次性完成
...
- SVC 作为 Preloaded Shaders（挂在 Graphics Settings）：对构建期变体生成没有特殊影响，keyword 组合同样并入 `usedKeywords`，不是替代。它的作用是运行时自动加载和 WarmUp——Player 启动时 Unity 自动预热这些 SVC 里的变体，不需要代码手动调用。注意：只影响运行时预热时机，不影响哪些变体被生成。

补充说明——如果想让 SVC 完全替代 `usedKeywords` 枚举（只生成 SVC 里登记的变体），需要通过 `-svc-driven-build <svc_path>` 命令行参数触发特殊构建模式。注意：这个模式要求 Player Settings 里的 `Strict shader variant matching` 必须开启，否则会报错。这是 Unity 构建工具链的高级用法，日常项目不会直接接触到，和挂 Preloaded Shaders 是两回事。
...
| After settings filtering | 经过设置过滤和 usedKeywords 枚举后剩余 |
| After built-in stripping | Unity 内置 stripping 后剩余（全局 feature 不用的路径被剔） |
| After scriptable stripping | SRP + 自定义 stripping 后剩余 |

如果 `Full variant space` 和 `After settings filtering` 之间差异很大，说明 usedKeywords 收集的组合远少于理论空间——这是正常的精细收集，但也意味着热更包里的材质组合如果不在 usedKeywords 里就会被漏。
