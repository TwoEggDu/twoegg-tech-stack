...
即使有 CI 验证，线上仍然可能出现变体缺失。以下是从"用户报了粉色材质"到定位根因的最短路径：

**第一步：确认是 Variant 问题还是 Shader 编译问题**

开启 `Strict Shader Variant Matching`（Player Settings → Other Settings），重新打包测试。如果开启后粉色材质增多并且 logcat / Console 里出现了明确的 keyword 缺失日志，就是 Variant 问题。如果开启前后表现一致，可能是 Shader 编译错误或资源损坏。

**第二步：从日志里提取缺失的 keyword 组合**

Strict Matching 的日志格式是：`Shader 'XXX', pass 'YYY', keywords 'KW1 KW2 KW3' not found`。把这组 keyword 记下来。

**第三步：反查这组 keyword 为什么没被编译**

三个方向排查：
1. **Pipeline Asset 配置**：检查构建时生效的 Pipeline Asset 是否开启了对应功能。比如缺 `_SCREEN_SPACE_OCCLUSION` → 检查构建时的 Pipeline Asset 是否开了 SSAO
2. **AB 构建时用了哪个 Pipeline Asset**：AB 构建只看 Graphics Settings 里的默认 Asset。如果默认 Asset 是 Low 档但运行时切到了 High 档，High 档特有的 keyword 就不在 AB 里
3. **自定义 Stripping**：检查 `IPreprocessShaders` 回调是否误删了这组 keyword

**第四步：修复后验证**

修复方案确定后（加入 SVC / 修改构建脚本 / 调整 Pipeline Asset），重新构建一个 Development Build，开着 Strict Matching 跑一遍主要场景。日志里不再出现 keyword 缺失即可确认修复。
