...
> **读这篇之前**：本篇会用到 ScriptableObject 生命周期和 CommandBuffer 操作。如果不熟悉，建议先看：
> - [URP 深度前置 01｜CommandBuffer]({{< relref "rendering/urp-pre-01-commandbuffer.md" >}})
> - [怎么在 URP 里扩展渲染流程]({{< relref "rendering/unity-rendering-10-urp-extend.md" >}})
...
| 阶段 | 事件点 | 可以做什么 |
|------|--------|------------|
| 帧开始 | `BeforeRendering` | 几乎什么都没画，极少在这里插入 |
| **阴影** | `BeforeRenderingShadows` | 阴影 Map 绘制前 |
| | `AfterRenderingShadows` | **可以读/写 Shadow Map** |
| **预处理** | `BeforeRenderingPrePasses` | Depth Prepass / GBuffer 填充前 |
| | `AfterRenderingPrePasses` | **Depth / Normal 已写好，可以读取** |
| **G-Buffer** | `BeforeRenderingGbuffer` | Deferred 路径专用 |
| *(Deferred)* | `AfterRenderingGbuffer` | |
| | `BeforeRenderingDeferredLights` | |
| | `AfterRenderingDeferredLights` | |
| **不透明** | `BeforeRenderingOpaques` | 不透明物体绘制前（少用） |
| | `AfterRenderingOpaques` | **不透明画完，天空盒还没开始** |
| **天空盒** | `BeforeRenderingSkybox` | |
| | `AfterRenderingSkybox` | **天空盒画完，半透明还没开始** |
| **半透明** | `BeforeRenderingTransparents` | |
| | `AfterRenderingTransparents` | **所有几何体画完，后处理还没开始（最常用）** |
| **后处理** | `BeforeRenderingPostProcessing` | |
| | `AfterRenderingPostProcessing` | **URP 内置后处理完成后** |
| 帧结束 | `AfterRendering` | 帧结束，部分平台 RT 已 Resolve |
...
<!-- EXPERIENCE-TODO: 在完整示例前插入一段经验叙事。建议框架：
     "实际项目中，我们最常用 Renderer Feature 做的事包括 [2-3 个真实例子：
     角色受击闪白、场景转场径向模糊、低血量全屏红色叠加等]。
     最初尝试用后处理 Volume 来做，但 [遇到的问题：无法精确控制时机/
     需要逐角色独立控制/需要在半透明之前执行]。
     后来改成 Renderer Feature，[优势]。
     下面的灰度示例虽然简单，但它的骨架和我们项目里用的 [效果名] 完全一样——
     真正复杂的不是 Shader，而是 RT 生命周期和 Pass 时序的管理。"
-->
...
1. 在 Assets 里创建上面的 `MyFeature.cs` 和 `MyRenderPass.cs`
2. 创建一个新 Material，Shader 选 Universal Render Pipeline/Unlit，颜色设为灰色（这是临时验证用的，正式效果需要自定义 Shader）
3. 选中 Universal Renderer Asset → Add Renderer Feature → 找到 MyFeature
4. 把刚才的 Material 拖到 MyFeature 的 Settings → Material 槽位
5. 运行场景——如果一切正确，画面会被灰度化处理
6. 拖动 Intensity 滑块，观察效果强度变化
7. 如果画面没变化：检查 Material 是否为空、renderPassEvent 是否在正确的时机
...
- 上一篇：[URP 深度光照 03｜Ambient Occlusion：SSAO 实现原理、参数调参与移动端策略]({{< relref "rendering/urp-lighting-03-ambient-occlusion.md" >}})
- 下一篇：[URP 深度扩展 02｜RenderGraph 实战：Unity 6 的新写法]({{< relref "rendering/urp-ext-02-rendergraph.md" >}})
