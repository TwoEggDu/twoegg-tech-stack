...
weight: 1580
---
> **读这篇之前**：本篇会用到屏幕空间采样和法线贴图概念。如果不熟悉，建议先看：
> - [URP 深度光照 01｜URP 光照系统]({{< relref "rendering/urp-lighting-01-lighting-system.md" >}})
> - [Shader 核心光照 02｜法线贴图：TBN 矩阵与切线空间]({{< relref "rendering/shader-lighting-02-normal-map.md" >}})

Ambient Occlusion（环境光遮蔽）模拟的是：在角落、缝隙、物体相交处，光线更难照进来，所以这些地方应该更暗。它在视觉上显著增加场景的立体感和接触感——尤其是去掉后你才会明显感觉"画面变平了"。

<!-- DATA-TODO: 补充 SSAO 开/关对比截图（同一角落区域），以及中端 Android 上的帧时间差值。截图存放 static/images/urp/ssao-on-off-comparison.png -->
...
<!-- DATA-TODO: 用下面的表格模板替换上面的估算表。测试场景：室内场景（角落多），1080P。
     测试后把实测数据填入，删掉这段注释，取消下面表格的注释。

| 降质项 | 帧时间变化 (骁龙 870) | 帧时间变化 (iPhone 13) | 视觉影响描述 |
|---|---|---|---|
| 完整 SSAO (基线) | ms | ms | — |
| 开启 Downsample | -ms (-%) | -ms (-%) | |
| Source = Depth | -ms | -ms | |
| Blur Quality = Low | -ms (-%) | -ms (-%) | |
| 完全关闭 | -ms (全部回收) | -ms | |

> 测试设备：[设备型号]，Unity [版本]，URP [版本]。
-->

<!-- DATA-TODO: 在这里插入 SSAO 开/关对比截图。截图操作：
     1. 同一角落/柱子底部区域，SSAO 开启 → 截图
     2. 同一区域，SSAO 关闭 → 截图
     截图存放：static/images/urp/ssao-on.png, ssao-off.png
     插入格式：
     ![SSAO 开启：角落处有明显的接触阴影](/images/urp/ssao-on.png)
     ![SSAO 关闭：同一角落，立体感明显减弱](/images/urp/ssao-off.png)
-->
