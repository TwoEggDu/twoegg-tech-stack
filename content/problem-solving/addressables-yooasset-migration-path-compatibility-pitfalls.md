...
flowchart TD
    subgraph Migration["迁移覆盖的六个层面"]
        ...
        L5["缓存目录<br/>Unity Caching ↔ CacheSystem"]
...
- **YooAsset**：每次 `LoadAssetAsync` 创建独立的 ProviderBase。两个调用者拿到的 handle 各自独立，bundleLoader 层面的引用计数分别递增。
...
gantt
    ...
    axisFormat  Week %W
    section Phase 1 · 准备（Week 1-3）
    搭建抽象层 + 新框架构建管线     :a1, 2024-01-01, 14d
    section Phase 2 · 地址迁移（Week 3-5）
    地址映射 + AssetReference 扫描  :a2, after a1, 7d
    新框架构建验证                  :a3, after a2, 7d
    section Phase 3 · 运行时验证（Week 5-7）
    适配层集成测试                  :a4, after a3, 7d
    引用计数语义验证                :a5, after a3, 7d
    section Phase 4 · 预发布（Week 7-8）
    Staging 环境部署 + 缓存迁移测试 :a6, after a5, 7d
    section Phase 5 · 发布（Week 8-9）
    生产环境桥接版本发布             :a7, after a6, 3d
    监控首次下载指标                 :a8, after a7, 7d
    section Phase 6 · 收尾（Week 9+）
    移除旧框架代码和依赖             :a9, after a8, 7d
```

以上时间为中型项目的参考估算，实际周期因项目规模和团队经验而异。
