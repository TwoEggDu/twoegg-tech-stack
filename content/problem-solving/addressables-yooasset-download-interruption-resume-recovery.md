...
UnityWebRequest 网络错误
  → DownloadHandlerFileOperation 检测到失败
...
| **网络超时（CDN 响应慢）** | `UnityWebRequest` 有默认超时（平台相关），超时后 `result = ConnectionError`。无内置重试。 | `DownloadHandlerFileOperation` 有超时检测，失败后检查 `failedTryAgain` 计数器决定是否重试。重试时可续传。 |
