...
## 小结
- **时间主权**：Game Loop 的本质是时间主权——谁定义一帧，谁决定世界怎么前进。和 UI 事件循环不同，UI 循环以"响应"为中心、可以停；Game Loop 以"进度"为中心、不能停。
- **稳定模拟 + 平滑显示**：fixed timestep、accumulator 和 interpolation 是把两者同时保住的关键——模拟用固定步长保证可复现，渲染用插值保证画面平滑。
- **统一的调度协议**：Game Loop 其实是在扮演一种调度协议，规定输入何时采样、模拟何时闭包、渲染何时提交。这个协议一旦统一，所有系统就共享同一个帧编号，调试、回放、网络同步、性能采样都能对齐到同一节拍。它和 UI event loop、async scheduler 职责不同，不能混用。
