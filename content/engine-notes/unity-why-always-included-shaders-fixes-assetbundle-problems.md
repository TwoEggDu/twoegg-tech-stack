...
文档还特别说明，这个设置会覆盖 `shader keyword declaration settings`，哪怕你用了 `shader_feature` 这种通常允许按需裁剪的声明方式，Unity 也会把这一整份 shader 的所有 keyword 组合都带进构建。

这就是为什么它很像一个粗暴但有效的兜底开关。
...
### 1. 构建时如果它在列表里，bundle 存的是引用

前面说过，这时候 bundle 在假设：

`加载我的 Player 肯定已经内置了这份 shader。`

### 2. 但如果加载时你把它从列表里删了，Player 里可能根本没有它

这时现场就会变成：

- bundle 里只有引用
- Player 里又没有真正的 shader
