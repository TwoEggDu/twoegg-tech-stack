...
series_intro: "这组文章处理的不是某个 Unity 菜单怎么点，而是把"你保存了一个 .cs 文件，到编辑器重新可用"这段等待时间里发生的事情拆开，以及点击 Build 之后发生了什么：asmdef 如何决定哪些脚本一起编，Roslyn 把 C# 变成什么，bee_backend 怎么调度增量编译，ILPP 为什么要偷偷改你的字节码，Domain Reload 是什么、为什么代价那么高，Mono 和 IL2CPP 打包路径如何分叉，以及如何通过 .asmdef 设计让增量编译更快。只有这张地图先立住，遇到编译慢、卡死、改代码进 Play Mode 等待这类问题时，才知道从哪里入手。"
series_reading_hint: "第一次读建议按 01 → 02 → 03 → 04 → 05 → 06 顺序读；如果你现在就是编译卡死，直接跳第 04 篇；想搞清楚 ILPP 是什么，直接看第 02 篇；想优化编译速度，直接看第 06 篇。"
---
