...
补充说明——如果想让 SVC 完全替代 `usedKeywords` 枚举（只生成 SVC 里登记的变体），需要通过 `-svc-driven-build <svc_path>` 命令行参数触发特殊构建模式。这是 Unity 构建工具链的高级用法，日常项目不会直接接触到，和挂 Preloaded Shaders 是两回事。
