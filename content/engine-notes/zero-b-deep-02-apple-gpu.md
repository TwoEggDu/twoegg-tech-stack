...
> **术语说明**：本文的 "Tile Memory" 特指 Apple GPU 的可编程片上存储（Metal imageblock API）。Mali 和 Adreno 同样有片上 Tile Buffer，但它不对 Shader 暴露为可读写的存储——它只是 TBDR 的内部实现细节。这是两个名称相似但能力不同的东西。
