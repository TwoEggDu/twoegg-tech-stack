...
前两篇案例（[HCLR-17｜async 崩溃的真正根因与两种修法]({{< relref "engine-notes/hybridclr-case-async-crash-root-cause-and-two-fixes.md" >}})、[HCLR-18｜`Dictionary<ValueTuple, 热更类型>` 的 MissingMethodException 与 object 替代法]({{< relref "engine-notes/hybridclr-case-dictionary-valuetuple-hotfix-type-missing-method.md" >}})）记录了 async 崩溃和 `Dictionary<ValueTuple, 热更类型>` 各自的根因和修法。这一篇不再拆案例，而是把这两次排查过程里反复要做的判断收成一张决策图：
...
如果你对 `LoadMetadataForAOTAssembly` 的运行时语义还没有稳定直觉，先回看 [HCLR-02｜HybridCLR AOT 泛型与补充元数据]({{< relref "engine-notes/hybridclr-aot-generics-and-supplementary-metadata.md" >}})；如果你对 `link.xml`、`AOTGenericReferences.cs`、`AssembliesPostIl2CppStrip` 这些生成物还没有整体地图，先回看 [HCLR-03｜HybridCLR 工具链拆解]({{< relref "engine-notes/hybridclr-toolchain-what-generate-buttons-do.md" >}})。
...
- 上一篇：<a href="{{< relref "engine-notes/hybridclr-case-dictionary-valuetuple-hotfix-type-missing-method.md" >}}">HybridCLR 案例｜Dictionary\<ValueTuple, 热更类型\> 的 MissingMethodException 与 object 替代法</a>
...
- 基础回链：<a href="{{< relref "engine-notes/hybridclr-aot-generics-and-supplementary-metadata.md" >}}">HybridCLR AOT 泛型与补充元数据｜为什么代码能编译，到了 IL2CPP 运行时却不一定能跑</a>
