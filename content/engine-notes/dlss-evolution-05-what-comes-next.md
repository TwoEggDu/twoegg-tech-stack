...
写到这里，最偷懒的结尾方式，是说一句“DLSS 未来会更强”。这当然没错，但也几乎等于没说。尤其在 `2026-03-16` DLSS 5 已经正式公开之后，这个问题更应该改写成：当 NVIDIA 开始把 DLSS 定义成 real-time neural rendering model，游戏图形接下来会朝哪个方向继续演进？
...
DLSS 只是这场转向里最早、最显眼、也最成功的消费级入口。但它不会是终点。DLSS 5 已经表明，NVIDIA 想把“更高 FPS”继续推向“更高 visual fidelity”。真正的终点更像是一种新常识：渲染器负责生成可信基础信号，模型负责把这些信号组织成更完整的图像、更连续的帧序列和更可承受的高端光照体验，甚至开始直接参与材质与光照观感的提升。
