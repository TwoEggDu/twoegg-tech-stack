...
- [《数据导向运行时 03｜Structural Change、Command Buffer 与同步点：为什么改结构总是贵》]({{< relref "engine-notes/data-oriented-runtime-03-structural-change-command-buffer-and-sync-points-why-structural-mutations-are-expensive.md" >}})
  讲了为什么结构变化必须被收束。
- [《数据导向运行时 04｜调度怎么做：Burst/Jobs、Mass Processor、自己手搓执行图》]({{< relref "engine-notes/data-oriented-runtime-04-how-scheduling-works-burst-jobs-mass-processor-and-self-built-execution-graph.md" >}})
  讲了为什么系统还需要显式执行骨架。
- [《数据导向运行时 05｜构建期前移怎么做：Baking、Traits / Templates / Spawn、离线转换》]({{< relref "engine-notes/data-oriented-runtime-05-why-build-time-keeps-moving-forward-baking-traits-templates-spawn-and-offline-conversion.md" >}})
