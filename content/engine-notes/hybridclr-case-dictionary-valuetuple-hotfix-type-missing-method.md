...
注意：对值类型的泛型参数，`object` **不能**替代——值类型的 AOT 实现不共享，必须写出具体类型。上面这个案例之所以只需要 `object`，是因为需要 AOT 实例化的是**值类型键**（`ValueTuple<int,int>`），而热更类型只是**引用类型值**，值参数可以用 `object` 覆盖。
...
- 上一篇：<a href="{{< relref "engine-notes/hybridclr-case-async-crash-root-cause-and-two-fixes.md" >}}">HybridCLR 案例续篇｜async 崩溃的真正根因与两种修法</a>
