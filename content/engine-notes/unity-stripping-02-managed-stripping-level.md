...
Unity 自己在 `ManagedStrippingLevel.mem.xml` 里，对这几个枚举的描述其实已经给了很多关键信号。
...
在 `UnityLinkerArgumentValueProvider.cs` 里，映射关系是：
...
先看 Editor UI。`PlayerSettingsEditor.cs` 里把可选级别分成了两组：
...
再往后看 `IL2CPPUtils.cs`，逻辑更直接：

如果某种情况下 `IL2CPP` 真拿到了 `ManagedStrippingLevel.Disabled`，代码会把它强制改成 `Minimal`，然后再继续跑 `AssemblyStripper.StripAssemblies(...)`。
...
`EditorOnlyPlayerSettings.cpp` 现在的默认逻辑，的确就是这么写的。

但 Unity 还处理了一个兼容旧项目的迁移分支。

同一个文件里，序列化升级逻辑明确写着：

- `2020.3` 及更早版本，`IL2CPP` 默认是 `Low`
- `2021.2` 及以后，默认改成了 `Minimal`

为了不让老项目一升级 Unity 就突然改变 strip 行为，Unity 在读取旧项目时，如果发现当时没有序列化 managed stripping level，就会主动把旧默认值 `Low` 写回去。
