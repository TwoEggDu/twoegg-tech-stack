...
weight = 61
featured = false
tags = ["Unity", "Shader", "URP", "Variant", "Build"]
...
```
After builtin stripping: N   ← 包含了 URP 内置 stripping
After IPreprocessShaders: M  ← 自定义 stripping
```

URP 内置 stripping 的结果会在 `After builtin stripping` 里体现，和 Unity 的全局 feature stripping（雾效、光照贴图等）在同一个数字里。
