...
- [Unity 脚本编译管线系列索引｜从改一行代码到编辑器可用，中间发生了什么]({{< relref "engine-notes/unity-script-compilation-pipeline-series-index.md" >}})
...
19. [HybridCLR 修法决策｜DisStripCode、link.xml、补充元数据分别在什么时候用]({{< relref "engine-notes/hybridclr-fix-decision-disstrip-linkxml-metadata.md" >}})
   这一篇不再拆单个案例，而是把 HCLR-17、HCLR-18 里反复出现的判断条件收成一张决策图，专门回答 `DisStripCode`、`link.xml` 和补 metadata 到底该怎么分流。

20. [HybridCLR 共享类型判断｜为什么报错是 ValueTuple<int,string>，你写的却可能是 ValueTuple<int,object>]({{< relref "engine-notes/hybridclr-sharing-type-judgment-valuetuple-int-string-valuetuple-int-object.md" >}})
   这一篇承接 HCLR-19，专门把“日志里看到的具体类型”和“DisStripCode 里真正该写的共享类型”拆开。

21. [HybridCLR DisStripCode 写法手册｜值类型、引用类型、嵌套泛型、委托分别该怎么写]({{< relref "engine-notes/hybridclr-disstripcode-writing-patterns-valuetype-reftype-nestedgeneric-delegate.md" >}})
   这一篇不再讲分流判断，而是把 `DisStripCode` 这一步真正该怎么写，收成一套可复用的写法规约。

22. [HybridCLR AOTGenericReferences、DisStripCode、补 metadata 到底怎么配合]({{< relref "engine-notes/hybridclr-aotgenericreferences-disstripcode-metadata-how-to-work-together.md" >}})
   这一篇把三者的职责边界和协作顺序拉直，避免把它们混成同一种“泛型修法”。

23. [HybridCLR AOT 泛型高频坑型录｜UniTask、LINQ、Dictionary、委托、自定义泛型容器怎么排]({{< relref "engine-notes/hybridclr-aot-generic-pitfall-patterns-unitask-linq-dictionary-delegate-custom-container.md" >}})
   这一篇不再按单个案例拆，而是把项目里最常见的 AOT 泛型坑按坑型归档。

24. [HybridCLR AOT 泛型回归防线｜怎么把这些坑前移到 Generate、CI 和构建检查里]({{< relref "engine-notes/hybridclr-aot-generic-guardrails-generate-ci-build-checks.md" >}})
   这一篇把前面几篇的判断重新收回工程流程，避免问题只在线上或发包后暴露。
...
### 19. 你已经看过单点案例，但想把 `DisStripCode`、`link.xml`、补 metadata 的适用边界压成一张判断图

先看：

- [HybridCLR 修法决策｜DisStripCode、link.xml、补充元数据分别在什么时候用]({{< relref "engine-notes/hybridclr-fix-decision-disstrip-linkxml-metadata.md" >}})

### 20. 你已经知道该走 `DisStripCode`，但总被报错里的具体类型名带偏

先看：

- [HybridCLR 共享类型判断｜为什么报错是 ValueTuple<int,string>，你写的却可能是 ValueTuple<int,object>]({{< relref "engine-notes/hybridclr-sharing-type-judgment-valuetuple-int-string-valuetuple-int-object.md" >}})

### 21. 你已经判断该走 `DisStripCode`，但不知道代码到底该怎么写

先看：

- [HybridCLR DisStripCode 写法手册｜值类型、引用类型、嵌套泛型、委托分别该怎么写]({{< relref "engine-notes/hybridclr-disstripcode-writing-patterns-valuetype-reftype-nestedgeneric-delegate.md" >}})

### 22. 你总把 `AOTGenericReferences`、DisStripCode、补 metadata 混成一件事

先看：

- [HybridCLR AOTGenericReferences、DisStripCode、补 metadata 到底怎么配合]({{< relref "engine-notes/hybridclr-aotgenericreferences-disstripcode-metadata-how-to-work-together.md" >}})

### 23. 你不想每次都从一条日志重猜这到底是哪一种 AOT 泛型坑
...
### 24. 你想把这些判断前移到 Generate、构建和 CI，而不是等线上报错再补

先看：

- [HybridCLR AOT 泛型回归防线｜怎么把这些坑前移到 Generate、CI 和构建检查里]({{< relref "engine-notes/hybridclr-aot-generic-guardrails-generate-ci-build-checks.md" >}})
...
- 修法决策图：`DisStripCode`、`link.xml`、补充 metadata 的边界与分流
- 共享类型判断：报错里的具体类型和 `DisStripCode` 里真正该写的共享类型怎么翻译
- `DisStripCode` 写法规约：值类型、引用类型、嵌套泛型、委托分别怎么落到 AOT 代码里
- `AOTGenericReferences`、DisStripCode、补 metadata 的职责边界和协作顺序
- AOT 泛型高频坑型归类：`UniTask`、`LINQ`、`Dictionary`、委托、自定义泛型容器
- AOT 泛型回归防线：怎么把这些判断前移到 `Generate`、构建和 CI 流程里
...
如果你现在正卡在 AOT 泛型修法这条线上，可以直接顺着下面这组专题继续往下读：

- [HCLR-20｜HybridCLR 共享类型判断｜为什么报错是 ValueTuple<int,string>，你写的却可能是 ValueTuple<int,object>]({{< relref "engine-notes/hybridclr-sharing-type-judgment-valuetuple-int-string-valuetuple-int-object.md" >}})
- [HCLR-21｜HybridCLR DisStripCode 写法手册｜值类型、引用类型、嵌套泛型、委托分别该怎么写]({{< relref "engine-notes/hybridclr-disstripcode-writing-patterns-valuetype-reftype-nestedgeneric-delegate.md" >}})
- [HCLR-22｜HybridCLR AOTGenericReferences、DisStripCode、补 metadata 到底怎么配合]({{< relref "engine-notes/hybridclr-aotgenericreferences-disstripcode-metadata-how-to-work-together.md" >}})
- [HCLR-23｜HybridCLR AOT 泛型高频坑型录｜UniTask、LINQ、Dictionary、委托、自定义泛型容器怎么排]({{< relref "engine-notes/hybridclr-aot-generic-pitfall-patterns-unitask-linq-dictionary-delegate-custom-container.md" >}})
- [HCLR-24｜HybridCLR AOT 泛型回归防线｜怎么把这些坑前移到 Generate、CI 和构建检查里]({{< relref "engine-notes/hybridclr-aot-generic-guardrails-generate-ci-build-checks.md" >}})
