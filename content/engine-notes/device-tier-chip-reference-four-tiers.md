...
[每档资产规格清单：贴图压缩、LOD 与包体分层]({{< relref "engine-notes/device-tier-asset-spec-texture-and-package.md" >}})

如果你需要把档位判断接进线上治理（遥测、灰度、回滚），可以看：
[机型分档怎样接线上：遥测回写、Remote Config、灰度与回滚]({{< relref "engine-notes/urp-platform-03-online-governance.md" >}})
