...
`HybridCLR 的高级能力选型，不是“哪个更强就上哪个”，而是先判断你现在缺的是动态装载主链、metadata 可见性、generic 覆盖率上限，还是“修改现有 AOT 模块且保住未改动部分 AOT 性能”的能力。`
...
`社区版主线负责把热更真正跑通，补 metadata 负责把 AOT metadata 可见性补回来，Full Generic Sharing 负责把 generic 覆盖率上限往上抬，DHE 负责把“修改现有 AOT 模块”这件事做成函数级混合执行；真正稳的选型顺序，不是先挑最强能力，而是先判断你现在到底缺的是哪一层。`
