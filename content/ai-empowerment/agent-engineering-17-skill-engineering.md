...
- Anthropic Agent Skills API 通过 custom Skill ID 与可选 version 使用 Skill；截至 2026-08-25，custom Skill ID 为 `skill_...`，custom version selector 为 `skver_...` 或 `latest`，每个新版本都是完整 snapshot，必须重新上传完整文件集，省略的文件不会沿用。当前 Skills guide 的 prerequisites / examples 要求 API key 与 Code Execution tool，未展示或要求 `skills-2025-10-02` beta header。Managed Agents 是独立 beta surface，其 header 属于自身 API contract，不能推断为 Skills prerequisite。[Anthropic Agent Skills API Guide](https://platform.claude.com/docs/en/build-with-claude/skills-guide)、[Managed Agents Overview](https://platform.claude.com/docs/en/managed-agents/overview)
...
具体产品有不同管理面。Anthropic Agent Skills API 的 custom Skill ID 为 `skill_...`，custom version selector 为 `skver_...` 或 `latest`；每个新版本都是完整 snapshot，省略文件不会沿用。当前 Skills guide 未展示或要求 `skills-2025-10-02` beta header；Codex 本地 Skill 可以借助 Git 与 config disable，plugins 可承担分发；ChatGPT workspace 提供 owner、access、invocations、timestamps 与 delete。它们都不能被概括成一个开放格式自带的统一 registry 或 rollback 协议。
