...
slug: "agent-engineering-07-mcp-external-capability-boundary"
date: "2026-08-20"
description: "MCP 标准化外部能力的 discovery、schema、call、result、error 与 transport 消息边界；Host、Server 与 External System 仍需分别关闭执行、拒绝、结果和证据责任。"
draft: false
tags:
  - "Agent Engineering"
  - "AI Engineering"
...
前两篇已经建立了必要的前置边界：[Function Calling 与 Tool Use]({{< relref "ai-empowerment/agent-engineering-05-function-calling-tool-use.md" >}})说明 Tool Call 是结构化行动候选，不等于已经执行；[Tool Runtime]({{< relref "ai-empowerment/agent-engineering-06-tool-runtime.md" >}})则把 Schema、Policy、Execute、Result 与 Trace 拆开。本篇不重写完整 Runtime，而是把这些责任接到 Client / Server 的远程协议边界上。
...
这里延续的是课程已有词汇边界，而不是新定义。Tool 的当前含义、Host 的应用责任与 Evidence 的证明要求以[课程词汇表](https://github.com/TwoEggDu/twoegg-tech-stack/blob/main/docs/agent-engineering-course/glossary.md)为准。本篇只做到协议映射，不提前展开后续概念的完整机制。
