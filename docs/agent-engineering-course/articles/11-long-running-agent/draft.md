# Long-running Agent：Checkpoint、Retry、Cancellation 与 Recovery
...
[Article 10](../../../../content/ai-empowerment/agent-engineering-10-state-machine-workflow.md)已经建立 State、合法 Transition、Guard 与 Terminal 的确定性骨架。但 State 只回答“当前提交到哪里”，不会自动回答：哪些 Action 已完成，哪个副作用可能已经发生，取消由谁发起，Retry Budget 还剩多少，下一次执行还能从哪里安全继续。
...
这正好接上前几篇的责任边界：[Article 06](../../../../content/ai-empowerment/agent-engineering-06-tool-runtime.md)让 Tool Outcome、取消来源和副作用执行 seam 可见，但其 single-process de-dup 没有关闭 crash window；[Article 08](../../../../content/ai-empowerment/agent-engineering-08-agent-loop.md)让每个 Step 提交 State，并把 `STOPPED` 与 `SUCCEEDED`分开，但没有实现 Recovery；[Article 09](../../../../content/ai-empowerment/agent-engineering-09-planning.md)让 Plan 保持候选身份，却不负责把计划恢复成运行事实。
...
- [Article 11 Evidence Register](evidence.md)
- [Lab 04 Design / Observation / Interpretation](../../labs/lab-04-state-machine-checkpoint/README.md)
- [Lab 04 Execution Log](../../labs/lab-04-state-machine-checkpoint/observations/execution-log.md)
- [Lab 04 Verification Summary](../../labs/lab-04-state-machine-checkpoint/observations/verification-summary.json)
- [Lab 04 run-a process evidence](../../labs/lab-04-state-machine-checkpoint/observations/process-evidence-run-a.json)
- [Lab 04 run-b process evidence](../../labs/lab-04-state-machine-checkpoint/observations/process-evidence-run-b.json)
