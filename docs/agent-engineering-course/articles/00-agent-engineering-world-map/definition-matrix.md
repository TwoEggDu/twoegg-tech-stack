...
| Product | 面向用户交付的软件 / 产品边界；可以使用或暴露一个或多个 Host，不等同某个具体运行入口。 | `PRODUCT_TERM` | Medium-High | 00 |
...
Host：CLI / IDE / Web / Desktop / CI / Unity Editor
↓
Harness
↓
Agent Runtime
...
这是一张课程导航图，用于安排学习顺序。`Product / Application` 表示面向用户的软件边界，并不主张 Product 与 Application 在所有语境下严格同义；`Host` 是课程对具体运行 / 集成入口的工作定义。整张图不是所有 Agent 产品的统一部署架构，也不能用于反推 Claude Code、Codex 或 DeepSeek Harness 的内部实现。
