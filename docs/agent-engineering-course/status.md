...
- Factory Run State：[course-run-state.md](course-run-state.md)（Article 22 targeted repair=`PRE_COMMIT_RECONCILIATION`；第一次fix commit/push/remote verify后才允许fresh Part IV targeted re-audit；Article 23 / 24 zero assets）
...
- 下一条允许动作：完成Article 22 targeted repair的显式diff/stage验证，创建独立fix commit并single push/remote verify；其后仅允许fresh Part IV targeted re-audit。
...
| 22 | Eval、Golden Dataset 与 Regression：修复以后还会不会再坏 | IV | L | 否 | `PUBLISHED / COMPLETED / END_ARTICLE / POST_PUBLICATION_FIX CANDIDATE` | `PASS / 13 of 13 / 3 CONFIRMED / 7 PARTIAL / 3 PROPOSAL / 0 BLOCKED / 13 Cards` | Lab 06 `VERIFIED / EVIDENCE_MERGED / AC-01..AC-10 / FROZEN ARTIFACTS PRESERVED` | Post-publication Final=`PASS / 94 / IR22-F01—F04 CLOSED / 0 OPEN`；Hugo=`1251 / 0 WARNING / 0 ERROR`；Published exact identity=`PASS`；original completion commit=`99bff931b02356358edd1357c2abd1c44621e720` remains historical evidence；repair commit/push=`PENDING` |
