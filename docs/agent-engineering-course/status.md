...
- Factory Run State：[course-run-state.md](course-run-state.md)（`PAUSED / EXPLICIT_HUMAN_STOP_LINE / Article 23 PRECHECK pointer / FORBIDDEN / active NONE`；待Part IV Audit checkpoint diff / commit / push / remote verification后STOP；Article 23 / 24 zero assets）
...
- 下一条允许动作：只读Git diff与显式stage验证，创建唯一`Audit Agent Engineering Part IV` commit并single push，完成remote verification后立即STOP
...
| 22 | Eval、Golden Dataset 与 Regression：修复以后还会不会再坏 | IV | L | 否 | `PUBLISHED / COMPLETED / END_ARTICLE` | `PASS / 12 of 12 / 3 CONFIRMED / 6 PARTIAL / 3 PROPOSAL / 0 BLOCKED / 12 Cards` | Lab 06 `VERIFIED / EVIDENCE_MERGED / AC-01..AC-10` | Final=`PASS / 95 / A22-R0-F01 CLOSED / 0 OPEN`；Hugo=`1251 / 0 WARNING / 0 ERROR`；Published exact identity=`PASS`；completion commit=`99bff931b02356358edd1357c2abd1c44621e720`；local / origin / live remote equality=`PASS` |
