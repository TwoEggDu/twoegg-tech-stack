...
2. 热修复流程：紧急修复的快速验证通道、Cherry-Pick 策略、回滚预案与演练 `待写`
3. Crash / ANR / 卡顿监控：版本健康看板怎样立 `已写，Crash 系列可复用`
4. Bug 管理与 Playtest：严重级别、生命周期、验证闭环 `待写`
5. 兼容性测试策略：设备矩阵划定、优先级和 Device Farm 边界 `待写`
...
- 版本怎样灰度放量、怎样盯线上健康、什么时候该回滚

还没补上的，主要是热修复、Bug 管理、兼容性测试，以及几篇更强的团队机制收束文。

## 推荐后续写作顺序

1. 先补 `热修复流程：紧急修复的快速验证通道、Cherry-Pick 策略、回滚预案与演练`
原因：这是灰度发布和线上监控之后最自然的一篇，直接回答线上出问题以后怎样安全地修。

2. 再补 `Bug 管理与 Playtest：严重级别、生命周期、验证闭环`
原因：把线上和灰度期发现的问题继续接回团队日常验证流程。

3. 再补 `兼容性测试策略：设备矩阵划定、优先级和 Device Farm 边界`
原因：把线上质量从“单一大盘”继续推进到设备和平台差异这一层。

4. 再补 `从规范到护栏：命名、目录边界、配置契约、脚手架分别解决什么`
原因：把已经写出的规则和流程收束成团队能长期执行的机制。

5. 最后补 `变体、资源、热更链路怎样接进 CI`
原因：把当前已经散在多个系列里的工程实践真正收成一篇总收束文。
