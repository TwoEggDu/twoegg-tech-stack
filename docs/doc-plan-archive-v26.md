﻿# 归档说明：这是 2026-04-18 迁出的历史篇级总表；当前入口请使用根 `doc-plan.md`。
...
*读引擎源码和写大型游戏系统的通用认知框架。分两条线组织：**教科书线**（引擎无关、语言中立）+ **应用线**（Unity 游戏、构建系统、未来扩展）。*
...
*这条系列不是 C# 语法入门，而是贯通 `设计模式前置知识 → ECMA-335 → CoreCLR → IL2CPP / HybridCLR / LeanCLR` 的入口主线。先把值类型、引用类型、object、string、class / struct / record、装箱拆箱这些概念讲清，再下钻到 CLI 规范、CoreCLR 参考实现和跨运行时差异。详细规划见 [docs/csharp-to-clr-series-plan.md](docs/csharp-to-clr-series-plan.md)。*

| 编号 | 标题 |
|------|------|
| CCLR-00 | 从 C# 到 CLR：这条线到底在讲什么 |
| CCLR-01 | 值类型、引用类型、对象：先把 3 个最容易混的词讲清楚 |
| CCLR-02 | int、bool、enum、char、decimal：内建类型不是“特殊语法”，而是运行时约定 |
| CCLR-03 | string 和 object：一个最特殊，一个最基础 |
| CCLR-04 | class、struct、record：三种边界，不是三种写法 |
| CCLR-05 | 装箱与拆箱：什么时候只是转换，什么时候真的产生对象 |
| CCLR-06 | 从 C# 到 CLI：语言前端、CTS、CLS 到底怎么对应 |
| CCLR-07 | 值类型与引用类型在 ECMA-335 里怎么定义 |
| CCLR-08 | 方法、字段、属性、事件在 metadata 里各长什么样 |
| CCLR-09 | 泛型、约束、签名：CLI 怎样表达“类型参数” |
| CCLR-10 | 对象在 CoreCLR 里怎么存在：对象头、MethodTable、字段布局 |
| CCLR-11 | 值类型到底在哪里：栈、堆、寄存器和“值类型都在栈上”的误解 |
| CCLR-12 | virtual、interface、override：多态分派到底怎么跑 |
| CCLR-13 | string、数组、Span：连续内存、只读语义和性能边界 |
| CCLR-14 | CoreCLR vs IL2CPP：同样的 C#，为什么会走向两种完全不同的执行模型 |
| CCLR-15 | 从 AOT 到热更新：为什么 HybridCLR 要补 metadata、解释器和 bridge |
| CCLR-16 | 从零造 CLR：LeanCLR 为什么选择另一条路 |
| CCLR-17 | 同一组 C# 语义，在不同 runtime 里分别牺牲了什么 |
...
系列十九（插件）   系列十九·B（HybridCLR）   系列十九·C（从 C# 到 CLR）
...
*最后更新：2026-04-18（v26）——新增“从 C# 到 CLR”入口主线规划（18 篇），用于串联设计模式前置知识、ECMA-335、CoreCLR、IL2CPP、HybridCLR 与 LeanCLR。总计约 671 篇。*
