...
> 初版：2026-09-20；定向修订：2026-09-21；首批教程：2026-09-22。状态：**3 篇入门正文已写；普通 C++ 小练习已本地验证；UE 工程、源码与个人能力尚未验证**。
...
2026-09-21 完成计划修订与 M0 执行计划；2026-09-22 按本轮授权编写 UFS-02 首课和 UFS-03 两篇子课，普通 C++ 仅在临时目录验证。历史审计与原专题正文保留，不创建外部工程仓库、不安装大型环境、不执行 UE 工程任务，也不自动 commit/push/创建 PR。
...
证据清单沿里程碑增量扩展，不把后续字段提前设为必填。M0 将真实源码/资产/配置/构建规则身份 `source_revision`、教学场景标签 `tutorial_source_label`、每次构建尝试唯一的 `build_id` 和每次运行唯一的 `run_id` 分开；另记录引擎身份、工具链/OS、配置与命令，以及包/符号/运行日志的对应关系和期望/实际/限制。M1 加两客户端和 DS 身份；M2 加业务请求、对局、结算及数据库记录；M4/M5 加 Trace、负载、恢复与交付证据。具体规则见第 4.1 节和 M0 文件，失败尝试也保留。
...
| UFS-09@M0 | source_revision/补丁或快照、教学场景标签、引擎身份、build_id、run_id、配置、包、符号、实际 exe 与本地运行日志相互对应；不要求 DB、业务请求、网络对局或观测集群 | @M1 加两客户端/DS 的进程身份与日志关联；@M2 加请求、match_id、结算和 DB；@M4/M5 加性能 Trace、负载、恢复与交付证据 |
...
- **缺口**：UFS-03 的空工程基线：目录、模块、配置、C++/蓝图分工、日志、source_revision/教学场景标签/build_id/run_id、符号和包。
...
下列 UFS 编号只管理贯通和桥梁内容，保留 UFS-01–UFS-20。**UFS-02 的 M0 首课、UFS-03 的两篇子课已写，实际路径与验证状态见 7.4；其余单元仍待写**。7.1/7.2 继续描述单元级问题与验收，不把成稿等同于工程或个人能力通过。
...
### 7.4 首批正文与执行入口（2026-09-22）

沿用本站已有 `02b` 子课的表达方式，一个 UFS 内容单元可以对应多篇正文。UFS-03 的 A/B 是阅读子课，不新增 UFS 编号或第二份权威目录；站点系列统一为“UE 全栈工程师成长路线”。

| 内容单元 / 阅读顺序 | 已写正文的真实路径 | 服务的 M0 任务 | 技术验证与未完成边界 |
| --- | --- | --- | --- |
| UFS-02 首课 / 1 | [从 C# 到 C++：对象由谁持有，代码怎样变成程序？](../content/engine-toolchain/ufs-02-csharp-to-cpp-ownership.md) | M0-T02 的最小原生语言桥梁 | Visual Studio/MSVC 为 Windows 默认操作路径，命令与 GUI 调试步骤已按 Microsoft 官方文档核对但尚未在本机实跑；可选 GCC 8.3.0/GDB 8.2.1 路径的编译、链接、运行及故障断点修复已有历史实测。未在 UE 编译，不代表读者能力通过；后续并发范围仍按 4.1 |
| UFS-03 子课 A / 2 | [UE 工程地图：从 .uproject、模块到编译和打包](../content/engine-toolchain/ufs-03a-project-build-map.md) | M0-T01/T03/T05，衔接 T06/T08 的身份与产物 | 正文、最小模块/Target 文件与步骤已写；按官方 5.8 文档核对。5.8.2 创建工程、编译、打包、符号匹配待验证 |
| UFS-03 子课 B / 3 | [UE C++ 第一课：一个旋转 Actor 怎样连接代码、反射和蓝图？](../content/engine-toolchain/ufs-03b-first-rotating-actor.md) | M0-T04/T06/T07，与 A 共用 T05；为 UFS-04 铺路 | 完整 Actor 头/实现、蓝图/实例步骤、断点与时间单位故障已写并静态核对；UE 编译、画面、包内调试和回归均待运行 |

三篇均为教程正文，不是已测 UE 案例。先读 1→2 的工程创建部分→3 的 Actor/地图，再回 2 打包并继续 3 的独立包验证；始终使用同一个 TwinArena。M0 仍由[原执行计划](ue-fullstack-m0-execution-plan.md)验收，T01–T08 与个人能力不因文章成稿自动改成通过；UFS-09 的完整证据专题仍待写。
...
| 准备项 / 用途 | 最低可用条件 | 何时 / 已有或待确认 | 验证方式 | 可替代方案 |
| --- | --- | --- | --- | --- |
| UE 版本 / 可复现 | 用户选定安装目标 5.8.2；M0 仍须锁实际发行构建 ID 或源码 revision、插件清单 | M0 / 目标已选，实际安装/构建身份待确认 | 版本文件、构建标识与日志交叉核对；二进制发行版不伪造 Git SHA，源码构建 revision 到 M1 锁定 | 工具链按 9.1 的 5.8 新记录核对，不沿用历史 5.7 组合 |
...
**当前目标（2026-09-22）：用户选定安装 UE 5.8.2**。这不是已安装、编译器已匹配或 UE 工程已验证的声明。首批教程查阅的官方页面显示 5.8 文档，没有把 minor 文档写成 5.8.2 patch 实测；M0 仍需记录实际 Build.version/发行构建标识，源码 revision 在采用源码构建时锁定。

本次重新核对的 [5.8 VS 兼容页](https://dev.epicgames.com/documentation/unreal-engine/setting-up-visual-studio-development-environment-for-cplusplus-projects-in-unreal-engine)列 VS 2022 17.14+、VS 2026 18.0+，一般开发推荐 VS 2026；MSVC 最低 14.38、推荐 14.50；Windows SDK 最低 10.0.22621.0、推荐 10.0.26100 或更新。采用顶部版本兼容表，不沿用页内较旧组件说明的 SDK 下限；实际 5.8.2 选择/接受的组合还要由 UBT 日志和首次构建确认。Linux、DS、复制系统等未在本轮全面重核，到对应里程碑按实际版本再验。

下表保留 **2026-09-20 初版以 5.7.x 为候选时的核查记录**；2026-09-21 风险复核仍见 3.1。它们是带日期的历史依据，不作为当前 5.8.2 的兼容保证，不回写成“今天全部重验”。
...
下列入口保留初版核查日的资料导航；当时未指定版本的 Epic 页面显示 5.8，不能倒推为历史候选 5.7 的保证。2026-09-22 首批教程使用的 5.8 资料与未验证项在各正文单列；其他入口到实际使用时按 9.1 当前目标重核。源码入口只能在有权限和 revision 后补，不伪造文件/函数验证记录。
...
- M0 只维护 [现有执行入口](ue-fullstack-m0-execution-plan.md)，不再建 execution-v2/workorders/status 副本。本轮授权限于 7.4 的三篇正文、最小计划回写及临时普通 C++ 验证；UE 工程操作、安装、外部发布或 commit/push/PR 不在本轮范围。
- 第一版的未决项：个人测评、硬件/磁盘、UE 5.8.2 实际安装/构建身份与源码 revision/许可、Linux 环境、后端/DB 版本、Android 设备、预算/运行窗口、外部复验者。它们不阻塞教程成稿，但会阻塞对应工程关卡通过。

## 11. 首批优先准备的 9 个内容建设单元

以下保留**内容建设任务**的单元级目标；UFS-02/03 的首批正文状态与路径以 7.4 为准，其余仍待写。顺序是建设顺序，UFS-09 提前准备证据标准，不改变学习依赖，也不授权自动实现 UE 工程。
...
- **推动/验收**：M0 按执行计划完整打包、脱离编辑器运行，source_revision、教学场景标签、build_id、run_id、产物和符号可对应，本人能重复操作并定位/修复自有故障；跨环境他人复验到 M5。
...
2026-09-22：新增 7.4 的三篇可操作教程入口，更新当前安装目标为用户选定的 5.8.2，并重核官方 5.8 Windows 工具链资料。普通 C++ 与 UE 代码验证状态分列；M0 仅补阅读链接/版本提醒，根 doc-plan 路由、历史审计与原文章未改。
