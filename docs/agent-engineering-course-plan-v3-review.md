# Agent Engineering 课程规划 v3（能力生长版评审稿）
...
> 日期：2026-08-18
>
> 输入：游戏 Agent 工程系列提纲（评审稿）与 Agent Engineering 课程提纲 v2

---

## 0. 文档状态

本稿不是在旧 34 / 35 篇上调整顺序，而是按“能力为什么在此刻出现”重新推导课程。
...
让一个熟悉 C# / Unity 和传统软件工程、但 Agent 知识不系统的工程师，从 Model API 与 Structured Output 开始，亲手理解 Tool Runtime、Agent Loop、State、Context、Memory、RAG、Reliable Agent 与 Harness 为什么依次出现；再用锁定版本的 DeepSeek Harness 验证这些抽象，最终独立完成 BuildPilot Design v1。
...
08 Agent Loop / Turn / Step / Observation
  ↓
09 Planning / State / State Machine / Workflow
  ↓
10 Checkpoint / Long-running / Cancellation
  ↓
11 Context Engineering
  ↓
12 Context Debugging / Compression / Pollution
  ↓
13 Working Memory / Investigation State
  ↓
14 Session / Long-term / Project Memory
  ↓
15 Knowledge Base / RAG
  ↓
16 Skill Engineering
  ↓
17 Evidence Contract
  ↓
18 Permission / Approval / Sandbox
  ↓
19 Token / Step / Cost / Latency Budget
  ↓
20 Trace / Replay / Failure Taxonomy
  ↓
21 Eval / Golden Dataset / Regression
  ↓
22 Multi-Agent（后置）
  ↓
23-26 Harness Engineering
  ↓
27-35 DeepSeek Harness
  ↓
36-42 BuildPilot Design v1
~~~
...
|---|---|---|---|
| Model I/O | 01-04 | 30、31 | 39-42 |
| Tool Runtime | 05-07 | 33 | 37、40 |
| Loop / State / Workflow | 08-10 | 31-32、35 | 38、40 |
| Context / Memory / Knowledge | 11-16 | 30、32、35 | 38、40 |
| Reliable Agent | 17-22 | 32-34 | 37-42 |
| Harness | 23-26 | 27-35 | 39-42 |

## 9. Part I｜从 LLM 到可编程模型（01-04）

核心问题：模型如何从“生成文字”变成程序可以稳定调用、解析和替换的工程组件。

## 10. Part II｜从模型到 Agent（05-10）

核心问题：模型怎样表达行动，宿主怎样安全执行，Decide → Act → Observe 怎样形成 Loop，长任务怎样获得 State、Workflow 与 Checkpoint。

## 11. Part III｜Agent 的信息、状态与知识（11-16）

核心问题：Agent 每个 Step 看什么；当前任务状态、会话事实、长期经验、权威知识和按需方法分别放在哪里。

## 12. Part IV｜Reliable Agent Engineering（17-22）

核心问题：怎样把自然语言推断变成可审计数据，并为真实环境建立权限、成本、Trace、Eval 与回归。

## 13. Part V｜Harness Engineering（23-26）

核心问题：当 Model、Tool、Context、State、Policy、Trace、Eval 和 Recovery 同时存在时，由谁统一承载与组织。

## 14. Part VI｜DeepSeek Harness 源码教材（27-35）

核心问题：一个真实 Harness 怎样把前五部分的抽象实体化；每篇都从已有问题进入，不做目录考古。

## 15. Part VII｜BuildPilot Design（36-42）

核心问题：从真实游戏生产管线推导一个专用只读诊断 Agent，最终交付 Design v1，不在课程内完成 Runtime。

## 16. Engineering Labs 路线
...
| Lab 04 State Machine + Checkpoint | 10 后 | Fake Long-running Investigation | 状态转移、Resume、取消 | 重试副作用、丢失检查点 |
| Lab 05 Context Debugging | 12 后 | 正确 / 过期 / 冲突资料包 | Context Snapshot、污染、截断 | 正确信息被淹没 |
| Lab 06 Trace + Eval | 21 后 | Simple Investigation Golden Set | Trace、Failure Layer、Regression | 答案正确但证据伪造 |
...
30、31 会重新遇到 Model Adapter、请求装配和 Step 调用。
...
30 会把本篇映射到 System Prompt Assembly 与多来源 Section。
...
31、33 会重新遇到模型 Step 输出、Tool 参数和 Result Validation。
...
29、31 会研究 Provider / Capability Seam 和 Agent Step 如何调用 Model。
...
33 会回收到 Tool Registry 与 Pipeline 的模型侧入口。
...
33 对应 Tool Registry、Policy 与 Execution Pipeline。
...
29、33、35 会检查 Capability / Provider 与 Tool Registry 的连接点。
...
5. 最小 Loop  
   ...
   5.2 引出 Lab 03 和 09 的 Planning / State。
...
31 对应 Inbox、Turn、Step 与 Agent Loop。
...
### 09｜Planning、State Machine 与 Workflow：确定性骨架和开放决策点
...
08 的自由 Loop 能执行，但长任务容易漏步骤、重复和失去恢复点。需要把确定的状态和转换写进程序，只保留需要上下文判断的决策点。

#### 3. 学完以后应该能回答的问题

- Planning 是模型输出、显式数据还是 Workflow？
...
08 Agent Loop；传统状态机知识。

#### 5. 核心概念

Plan、State、State Machine、Workflow、Stage、Transition、Invariant、Decision Point、Checkpoint。
...
2. State Machine  
   ...
   2.2 材料：简单调查状态图。
3. Planning  
   3.1 隐式计划、显式 Plan、可修订计划。  
   3.2 论点：Plan 是候选，不绕过 Workflow Guard。
4. Workflow + Agent  
   4.1 Workflow 调 Agent、Agent 调受控 Workflow Tool、Code Orchestration。  
   4.2 材料：三种控制权时序图。
5. Checkpoint  
   5.1 为什么可恢复状态必须持久化。  
   5.2 引出 10 的长任务、取消和 Resume。

#### 8. Engineering Lab / 示例

本篇不设独立 Lab；Lab 04 在 10 后同时验证 State Machine 和 Checkpoint。

#### 9. 与 DeepSeek Harness 的关系

31-32 会观察 Loop 与 Session State；35 判断 Workflow 是核心事实还是扩展映射。
...
### 10｜Long-running Agent：Checkpoint、Retry、Cancellation 与 Recovery
...
09 已有 State Machine，但真实 Tool 和外部系统会超时、取消或跨时段返回。没有 Checkpoint 与恢复语义，Workflow 只能从头重跑。
...
06 Tool Runtime；08 Loop；09 State / Workflow。
...
32、34 对应 Session Event、Cancellation、Compaction 与 Recovery。
...
### 11｜Context Engineering：每一个 Step 到底应该看到什么
...
08-10 让 Agent 连续运行，随之产生历史、Tool Result、State 和 Checkpoint。新问题不是 Prompt 怎么写，而是下一 Step 应看到哪些信息。
...
02 Prompt；06 Tool Result；08 Step；09-10 State / Checkpoint。
...
30 对应 System Prompt / PromptContext，31-32 对应 Step History 与 Session。
...
### 12｜Context Debugging：Packing、Compression、Pollution 与可重建性
...
11 已能装配 Context，但正确来源也可能因顺序、截断、过期、冲突和历史污染而失效。Agent 错误不能默认归因于模型。
...
11 Context Assembly；10 Checkpoint。
...
30、34 对应 Prompt Assembly、History、Compaction 与 Trace。
...
### 13｜Working Memory 与 Investigation State：当前任务正在想什么
...
12 让我们看到 Context 每 Step 重建，但 Agent 仍需保存当前目标、已知证据、假设、待办和失败尝试。它们不是长期 Memory，而是当前任务工作状态。
...
08 State；10 Checkpoint；11-12 Context。
...
31-32 检查 DSH 的 Step / Session 能否映射 Working State；不预设同名一级对象。
...
### 14｜Session、Long-term Memory 与 Project Memory：事实、经验和作用域
...
13 只有当前任务工作状态。任务需要中断恢复、跨会话复用经验，并且不同项目不能共享全部信息，因此需要分生命周期和作用域。
...
10 Checkpoint；13 Working Memory。
...
32 对应 append-only Session Event；35 讨论 Memory / Skill / RAG 的扩展映射。
...
### 15｜Knowledge Base 与 RAG：Retrieve、Filter、Rerank、Inject、Cite
...
14 能保存会话和经验，但 Agent 还需要从大量外部文档、事故和规则中按需取得相关信息。
...
11 Context；14 Memory / Authority。
...
35 判断 Retrieval Tool 与 Context Provider 两种接法，并严格区分扩展设计与 DSH 内置事实。
...
### 16｜Skill Engineering：按需加载领域方法，而不是再堆一层 Prompt
...
15 解决“找什么事实”，但 Agent 还需要知道“这类任务通常怎样做”。长期把所有方法放进 Prompt 会造成 Context Bloat。
...
35 检查 Skill 是否为 DSH 一级能力，以及如何通过 Plugin / Context / Tool 映射。
...
### 17｜Evidence Contract：把自然语言推断变成可审计工程数据
...
03 Structured Output；08 Loop；13 Working State；15 Citation。
...
32、34 研究 Session / Trace 承载领域事件的方式；不假设 DSH 内置本合同。
...
### 18｜Permission、Approval、Human-in-the-loop 与 Sandbox
...
17 让 Agent 可以提出 Action。下一问题是哪些 Action 有权执行、何时需要人批准、即使出错能影响多大范围。
...
06 Tool Runtime；09 Workflow Gate；17 ProposedAction。
...
33-34 检查 Tool Policy、Sandbox、Approval 与取消的真实接点。
...
### 19｜Budget Engineering：Token、Step、Cost 与 Latency
...
10 Long-running；11-12 Context；15 RAG；18 Policy。
...
34 对应 Cost、Compaction 和 Budget Control。
...
### 20｜Trace、Replay 与 Failure Taxonomy：错误究竟发生在哪一层
...
08-19，尤其是 Context、Tool、Policy 和 Evidence。
...
32、34 对应 Session Event、Trace、Cancellation 与 Recovery。
...
### 21｜Eval、Golden Dataset 与 Regression：修复以后还会不会再坏
...
20 能解释失败，但还不能判断系统整体是否更好。需要 Golden Dataset、Evaluator、回归和真实证据渠道。
...
17 Evidence；19 Budget；20 Trace / Taxonomy。
...
34 研究 Trace / Recovery 接点；DSH 本身不自动提供 BuildPilot 领域 Eval。
...
### 22｜Single Agent、Subagent、Agent as Tool、Handoff 与 Multi-Agent
...
21 已能测出单 Agent 的 Context、权限、成本和失败瓶颈。现在才有证据判断职责拆分是否带来净收益。
...
06 Tool Runtime；09 Workflow；11 Context；18 Policy；20-21 Trace / Eval。
...
29、35 检查 Subagent Provider、Profile 与 Host 编排边界。
...
### 24｜Agent Runtime vs Harness：执行内核与工程控制面
...
23 说明横切能力需要承载者。新问题是哪些仍属于执行内核，哪些应由外层组合和约束；不同框架可能切法不同。
...
27-29 会检验 DSH 的实际切层，不把本篇定义强加给源码。
...
### 25｜Harness 的最小能力模型：Capability、Policy、Session、Trace 与 Recovery
...
24 已切开执行与控制，但还缺一个不会无限膨胀的 Harness 最小模型。
...
28-34 分别对应 Plugin、Profile、Context、Loop、Session、Tool 与运行控制。
...
### 26｜Harness 的设计取舍：可替换性、复杂度、Bloat 与演化
...
25 给出最小能力模型，但把每项都插件化会产生隐藏依赖、配置漂移和调试成本。需要用真实变化压力决定抽象。
...
23-25；现有 Harness Engineering 系列作为延伸。
...
### 27｜怎样把 DeepSeek Harness 当作 Evidence-first 源码教材
...
5. 阅读地图  
   ...
   5.2 为 28-35 建 evidence card 路由。
...
**BLOCKED：必须锁定 commit，并记录 build / test / run 状态后才能写 28-35。**
...
### 28｜Everything is a Plugin：插件内核如何承载 Capability 与生命周期

#### 1. 本篇定位

源码原理篇。用插件内核验证 25-26 的 Capability Composition 与取舍。

#### 2. 为什么现在学它

27 已固定证据。第一个源码问题是 DSH 怎样组合众多 Harness 能力，并让它们拥有 Scope 和可释放生命周期。
...
### 29｜Profile、Bundle、Provider 与 Capability Seam
...
28 有插件生命周期，但还不知道如何选择插件、叠加配置和替换 Provider。
...
### 30｜System Prompt Assembly 与 PromptContext：多来源 Context 怎样组成

#### 1. 本篇定位

源码 Context 篇。回收 02、11、12 的 Prompt / Context / Debugging 主线。

#### 2. 为什么现在学它

29 安装了不同能力，多个插件会同时贡献身份、变量、Tool Guidance、动态状态和历史，需要有序装配与冲突语义。
...
### 31｜Inbox、Turn、Step 与 Agent Loop
...
30 已知道每个 Step 的请求怎样装配，现在追踪外部事件怎样唤醒 Turn、模型怎样调用 Tool、何时继续或停止。
...
### 32｜Append-only Session Event：Replay、Resume、Fork 与 Projection

#### 1. 本篇定位

源码状态篇。回收 10、13、14、20 的 Checkpoint、Session、Working State 和 Replay。

#### 2. 为什么现在学它

31 持续产生模型、Tool、Policy 和状态事件。List<Message> 无法完整恢复、审计或构建不同视图。
...
### 33｜Tool Registry 与 Tool Execution Pipeline

#### 1. 本篇定位

源码执行篇。验证 05-07、18 的 Tool Runtime 抽象。

#### 2. 为什么现在学它

31-32 已有 Loop 与事件流，现在追踪一次 Tool Call 从 Registry 到 Policy、Execute、Result 和 Session 的全过程。
...
### 34｜Cost、Compaction、Trace、Cancellation 与 Recovery

#### 1. 本篇定位

源码运行控制篇。回收 10、12、19-21 的长任务与可靠性机制。

#### 2. 为什么现在学它

30-33 已连通 Context、Loop、Session 和 Tool。长 Run 需要 Budget、Compaction、Trace、Cancel 与 Recovery 在真实接点协作。
...
### 35｜RAG、Skill、Workflow、Subagent 与 Web / Headless：核心事实和扩展映射
...
28-34 已掌握 DSH 核心。课程中的 RAG、Skill、Workflow、Subagent 和多 Host 未必都有同名一级实现，必须从 seam 映射而非硬找类名。
...
**BLOCKED：汇总 27-34 evidence card 与官方扩展文档。**
...
### 36｜游戏生产问题空间：什么时候该写 Script、Rule、Workflow，什么时候才需要 Agent
...
### 37｜案例 A：Unity Compile Golden Fixture——设计一个可判定的诊断 Agent
...
### 38｜案例 B：启动性能调查——设计一个长链路、多假设 Agent
...
### 39｜从两个案例反推 BuildPilot Architecture：先找变化轴，再定模块

#### 1. 本篇定位

架构推导篇。用 37-38 暴露的共性与差异反推 BuildPilot Design v1，而非先画框架图。
...
### 40｜BuildPilot 的 Context 与 Capability 设计：让知识、技能和工具各就各位
...
### 41｜BuildPilot 的治理闭环：Evidence、Policy、Session、Trace、Budget、Recovery 与 Eval
...
### 42｜BuildPilot Design v1：设计评审、里程碑与退出条件
...
**编辑判断**：知识飞轮先作为本课程 Part III 与文章 40-41 的一条主线，不急着另开专栏。等积累出真实的“Trace → Review → Knowledge / Skill / Eval → Regression”案例后，再拆成独立系列；否则会过早滑向泛知识管理。

## 21. 推荐阅读顺序

首次学习严格按 01 → 42：

1. 01-10 建立从 Model API 到有状态 Agent 的可运行链。
2. 11-16 学会组织信息、状态、知识与领域方法。
3. 17-22 建立安全、可观测、可评价的可靠性闭环。
4. 23-26 再推导 Harness，避免把 Harness 学成名词清单。
5. 27-35 用 DSH 校准源码级理解。
6. 36-42 将知识收束为 BuildPilot Design v1。

已有 Agent 实践经验的读者可以先读 00、总图和每 Part 导读，但不建议跳过 11-21 后直接进入 Harness。

## 22. 推荐写作顺序

写作顺序不必等同阅读顺序：

1. 先冻结 27 的 pinned commit 与 Evidence Card，确认 DSH 材料可得。
2. 完成 36-38 的问题空间和两个案例设计，保证课程终点真实可追溯。
3. 写 01-10 和 Lab 01-04，建立最小可运行教学链。
4. 写 11-16 和 Lab 05，补齐 Working Memory、Context Debugging 与知识能力。
5. 写 17-21 和 Lab 06，形成 Trace / Failure / Eval / Regression 闭环。
6. 写 23-26，再依据证据完成 28-35；22 多 Agent 最后穿插。
7. 写 39-42，汇总为 Design v1；最后回写 00 和各 Part 导读。

## 23. 当前需要实验或证据、不得靠推测完成的文章

| 文章 | 必要证据 / 实验 | 未满足时状态 |
|---|---|---|
| 03-04 | 至少一个真实 Provider 的 Schema、流式和错误行为 | **BLOCKED** |
| 05-06 | Function Calling 往返与 Tool Runtime 失败注入 | **BLOCKED** |
| 10 | Checkpoint、取消、恢复最小实验 | **BLOCKED** |
| 12-15 | Context 污染、压缩、Working Memory、RAG 引用实验 | **BLOCKED** |
| 17-21 | Evidence、Trace、Replay、Failure、Eval、Regression 串联实验 | **BLOCKED** |
| 27-35 | DSH pinned commit、源码路径、调用链与必要运行实验 | **BLOCKED** |
| 37 | 可公开 Unity Golden Fixture、版本与编译输出 | **BLOCKED** |
| 38 | 指标口径、设备、版本和启动阶段事件；否则仅用合成数据 | **BLOCKED** |
| 40-41 | 真实 Trace 经审核进入 Knowledge / Skill / Eval 的闭环 | **BLOCKED** |
...
- [x] Agent Loop 先于 Workflow、Checkpoint 与恢复。
...
- [x] DSH 采用 pinned commit、Evidence Card 与 **BLOCKED** 机制。
- [x] Multi-Agent 位于可靠性基础之后。
- [x] 6 个 Engineering Lab 均可独立完成，不依赖 BuildPilot Runtime。
- [x] 42 篇文章均使用完整 14 字段 Article Card。
- [x] BuildPilot 只交付 Design v1，不进入 Runtime 实现。
- [x] 知识飞轮保留在课程主线，并设置独立成系列的触发条件。

## 26. 最短结论

这不是“34 篇文章换个顺序”，而是一条从可编程模型、Agent 运行机制、信息与知识、可靠性控制，到 Harness 源码阅读和领域 Agent 设计的完整能力链。课程的最终产物不是一个仓促实现的框架，而是一份经过两个案例约束、能够进入工程评审的 BuildPilot Design v1。
