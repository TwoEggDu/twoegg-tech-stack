...
series_summary: "把 build-time、runtime、排障和工程化边界放回一条 HybridCLR 链路里"
series_intro: "这组文章处理的不是单个按钮或配置项，而是从 build-time 到 runtime 的整条 HybridCLR 链路。它先把运行时主链、元数据、AOT 泛型和工具链拆开，再把这些能力收回项目落地和排障判断。"
series_reading_hint: "第一次进入这个系列，建议先按目录顺着主线读；带着具体问题来查时，再直接跳到对应文章。"
```
...
- `series_summary`：系列卡片上的一句话摘要。`Engine Notes` 列表页会优先读取它。
- `series_intro`：索引页里的专题介绍，用 1 到 2 段说明这组文章在处理什么问题。
- `series_reading_hint`：索引页里的补充阅读建议，通常只写 1 段。

推荐写法：

- 系列入口页正文直接写 `{{< series-directory >}}`。
- 这个 shortcode 会自动渲染专题介绍、阅读建议和当前系列的文章索引。
- 入口页的 `weight` 建议放在该系列正文之前，占住“00 位置”。
