...
系列二（Unity渲染）  系列三（优化）
        ↓                ↓
系列二十二（Unity 6 渲染管线升级实战）
系列二十三（Unity 6 运行时与工具链变化）
系列二十四（Unity 2022→6 升级决策指南）
```
...
## 系列二十二：Unity 6 渲染管线升级实战（8 篇）

*从 Unity 2022 到 Unity 6，渲染管线发生了哪些结构性变化。依赖系列二（Unity 渲染）和 URP 深度系列作为前置。*

| 编号 | 标题 | 状态 |
|------|------|------|
| U6R-01 | GPU Resident Drawer 原理：从 SRP Batcher 到自动 Instancing | 待写 |
| U6R-02 | GPU Resident Drawer vs SRP Batcher：性能模型对比与切换时机 | 待写 |
| U6R-03 | Deferred+ 在 URP 中的落地：移动端是否可用 | 待写 |
| U6R-04 | BIRP 正式 Deprecated：现有项目的迁移成本和策略 | 待写 |
| U6R-05 | HDRP 停更后的选型决策：哪些项目还值得用 HDRP | 待写 |
| U6R-06 | 从 URP 14.x 到 17.x：API 破坏性变更与适配清单 | 待写 |
| U6R-07 | Unity 6 Shader Variant 治理变化：与 2022 的 Diff | 待写 |
| U6R-08 | 渲染管线统一路线图解读：URP 吞并 HDRP 的技术可行性 | 待写 |

---

## 系列二十三：Unity 6 运行时与工具链变化（6 篇）

*Unity 6 在运行时模型、异步编程、资产管线、平台适配上的结构性变化。依赖异步运行时系列作为前置。*

| 编号 | 标题 | 状态 |
|------|------|------|
| U6T-01 | Awaitable vs UniTask vs Coroutine：三代异步方案的架构对比 | 待写 |
| U6T-02 | Unity 6 Awaitable 的 PlayerLoop 集成：帧时序与调度语义 | 待写 |
| U6T-03 | Content Pipeline 后台化：对 CI/CD 和团队协作的影响 | 待写 |
| U6T-04 | Android GameActivity 替代方案：对热更新和插件化的影响 | 待写 |
| U6T-05 | Unity 6.x 滚动更新策略：Breaking Changes 的应对框架 | 待写 |
| U6T-06 | Unity Sentis 推理引擎：端侧 AI 的实际能力边界 | 待写 |

---

## 系列二十四：Unity 2022 → Unity 6 升级决策指南（4 篇）

*面向技术决策者的升级成本收益分析和执行方法论。依赖系列二十二和二十三提供事实基础。*

| 编号 | 标题 | 状态 |
|------|------|------|
| U6G-01 | 升级 vs 不升级：成本收益分析框架 | 待写 |
| U6G-02 | 升级实战 Checklist：按模块的兼容性排查流程 | 待写 |
| U6G-03 | 版本锁定策略：6.0 LTS / 6.x 滚动 / 2022 LTS 的项目类型匹配 | 待写 |
| U6G-04 | 第三方依赖审计：核心库的 Unity 6 适配状态 | 待写 |
